# PLATFORM_OPERATION / PLATFORM_VALIDATE

## Purpose and completion condition

코드·계약·고객 여정 검증 업무를 전문적으로 완료하고 검증 가능한 산출물을 생성한다.

## ADMIN screen contract: 플랫폼 운영 - 코드·계약·고객 여정 검증

- Route: `/admin/system/process-workspace?process=PLATFORM_OPERATION&step=PLATFORM_VALIDATE`
- Responsible actor: `PLATFORM_OPERATOR`
- Business purpose: 코드·계약·고객 여정 검증 업무를 전문적으로 완료한다.
