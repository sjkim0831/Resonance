...
경계 승인·버전 확정 화면·API·DB 계약은 테넌트와 프로젝트 경계를 포함하고 실패 시 이전 상태로 복구 가능해야 한다.

## ADMIN screen contract: 경계 승인·버전 확정

- Route: `/admin/emission/organizational-boundary`
- Responsible actor: `APPROVER`
- Business purpose: 경계 승인·버전 확정 화면·API·DB 계약은 테넌트와 프로젝트 경계를 포함하고 실패 시 이전 상태로 복구 가능해야 한다.
- Entry condition: STEP_3_COMPLETED
- Completion condition: 경계 승인·버전 확정의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.
...
- API: ["POST /home/api/emission-projects/{id}/organizational-boundary/decision"]
...
## USER screen contract: 경계 승인·버전 확정
