...
내부거래 제거·통합 계산 화면·API·DB 계약은 테넌트와 프로젝트 경계를 포함하고 실패 시 이전 상태로 복구 가능해야 한다.

## ADMIN screen contract: 내부거래 제거·통합 계산

- Route: `/admin/emission/organizational-boundary`
- Responsible actor: `CALCULATOR`
- Business purpose: 내부거래 제거·통합 계산 화면·API·DB 계약은 테넌트와 프로젝트 경계를 포함하고 실패 시 이전 상태로 복구 가능해야 한다.
- Entry condition: STEP_2_COMPLETED
- Completion condition: 내부거래 제거·통합 계산의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.
...
## USER screen contract: 내부거래 제거·통합 계산
