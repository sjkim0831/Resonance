# GOVERNANCE_CHANGE / GOV_SIMULATE

## Purpose and completion condition

정상·안전 시나리오 검증 업무를 전문적으로 완료하고 검증 가능한 산출물을 생성한다.

## ADMIN screen contract: 기준·워크플로 변경 - 정상·안전 시나리오 검증

- Route: `/admin/system/process-workspace?process=GOVERNANCE_CHANGE&step=GOV_SIMULATE`
- Responsible actor: `VERIFIER`
- Business purpose: 정상·안전 시나리오 검증 업무를 전문적으로 완료한다.
