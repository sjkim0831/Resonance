...
for token in EXTERNAL_BLOCKED DESIGN_GAP IMPLEMENTATION_GAP TEST_GAP READY framework_current_business_e2e_evidence framework_professional_screen_readiness framework_process_design_assurance_matrix designReadyScreens implementationReadyScreens testReadyScreens RETIRED_DUPLICATE_ADMIN_FLOW "contract_status IN ('DESIGN_COMPLETE','VERIFIED')" 'implementationReadyScreens<.screenCount' 'testReadyScreens<.screenCount' 'approvedTestTypes<5' 'currentPassedSteps<.stepCount' '--gate'; do grep -Fq -- "$token" "$TARGET" || { echo "[process-closing-classification] FAIL missing=$token" >&2; exit 1; }; done
