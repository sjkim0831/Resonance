...
grep -q 'CARBONET_PREDEPLOY_BACKUP_MODE:-flyway-only' "$DEPLOY_SCRIPT"
grep -q 'backup_scope="flyway-transaction-only"' "$DEPLOY_SCRIPT"
grep -q 'refusing database change outside a newly added versioned Flyway migration while backup mode is flyway-only' "$DEPLOY_SCRIPT"
grep -q 'Flyway-only mode: full and targeted pre-deploy dumps skipped' "$DEPLOY_SCRIPT"
