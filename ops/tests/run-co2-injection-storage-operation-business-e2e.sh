...
STEPS=(CISO_PLAN CISO_OPERATE CISO_REVIEW)
PROJECT_ID="QA-CISO-$(date +%s)-$RANDOM"
EVIDENCE="$ROOT/var/test-evidence/co2-injection-storage-operation-latest.json"
...
carbonet_postgres_query "insert into framework_account_actor_assignment(account_id,tenant_id,project_id,actor_code,data_scope,assignment_status)
  values ('qacalc26','TEST_COMPANY_001','$PROJECT_ID','STORAGE_SITE_MANAGER','$PROJECT_ID','ACTIVE'),
...
[[ "$RUNTIME_COMMIT" =~ ^[0-9a-f]{40}$ ]] || { echo "CISO_E2E_RUNTIME_IDENTITY_INVALID" >&2; exit 3; }

CARBONET_RELAY_QA_PROJECT_ID="$PROJECT_ID" \
CARBONET_RELAY_PROCESS_CODE="$PROCESS" \
CARBONET_RELAY_STEPS="CISO_PLAN,CISO_OPERATE,CISO_REVIEW" \
CARBONET_RELAY_STEP_ACTORS="STORAGE_SITE_MANAGER,STORAGE_SITE_MANAGER,HSE_MANAGER" \
CARBONET_RELAY_ACCOUNTS_JSON='{"STORAGE_SITE_MANAGER":"qacalc26","HSE_MANAGER":"qaverify26"}' \
CARBONET_RELAY_ROUTE="/ccus/facility/co2-injection-storage-operation" \
...
[[ "$residue" == 0 ]] || { echo "CISO_E2E_CLEANUP_FAILED residue=$residue" >&2; exit 3; }
...
for step in "${STEPS[@]}"; do
  ...
  jq -c --argjson contract "$contract" '.contract=$contract' "$EVIDENCE" | \
    ...
    bash "$ROOT/ops/scripts/promote-screen-contract-after-e2e.sh" "$PROCESS" "$step" "$required" USER >/dev/null
done
