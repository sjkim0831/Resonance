...
PROCESS="CERTIFICATE_FEE_TAX_REFUND"
STEPS=(CFTR_BILL CFTR_SETTLE CFTR_REFUND)
PROJECT_ID="QA-CFTR-$(date +%s)-$RANDOM"
EVIDENCE="$ROOT/var/test-evidence/certificate-fee-tax-refund-latest.json"
...
carbonet_postgres_query "insert into framework_account_actor_assignment(account_id,tenant_id,project_id,actor_code,data_scope,assignment_status)
  values ('qacalc26','TEST_COMPANY_001','$PROJECT_ID','SETTLEMENT_OPERATOR','$PROJECT_ID','ACTIVE')
...
[[ "$RUNTIME_COMMIT" =~ ^[0-9a-f]{40}$ ]] || { echo "CFTR_E2E_RUNTIME_IDENTITY_INVALID" >&2; exit 3; }
CARBONET_RELAY_QA_PROJECT_ID="$PROJECT_ID" \
CARBONET_RELAY_PROCESS_CODE="$PROCESS" \
CARBONET_RELAY_STEPS="CFTR_BILL,CFTR_SETTLE,CFTR_REFUND" \
CARBONET_RELAY_STEP_ACTORS="SETTLEMENT_OPERATOR,SETTLEMENT_OPERATOR,SETTLEMENT_OPERATOR" \
CARBONET_RELAY_ACCOUNTS_JSON='{"SETTLEMENT_OPERATOR":"qacalc26","HSE_MANAGER":"qaverify26"}' \
CARBONET_RELAY_ROUTE="/work/certificate-fee-tax-refund" \
CARBONET_RELAY_EVIDENCE_FILE="$EVIDENCE" RESONANCE_ROOT="$ROOT" \
...
[[ "$residue" == 0 ]] || { echo "CFTR_E2E_CLEANUP_FAILED residue=$residue" >&2; exit 3; }
...
for step in "${STEPS[@]}"; do
  ...
  jq -c --argjson contract "$contract" '.contract=$contract' "$EVIDENCE" | E2E_DEPLOYED_COMMIT="$RUNTIME_COMMIT" E2E_VALIDATION_COMMIT="$VALIDATION_COMMIT" bash "$ROOT/ops/scripts/promote-screen-contract-after-e2e.sh" "$PROCESS" "$step" "$required" USER >/dev/null
done
printf '{"status":"PROMOTED","processCode":"%s","steps":3,"cleanup":true,"sourceCommit":"%s","validationCommit":"%s"}\n' "$PROCESS" "$RUNTIME_COMMIT" "$VALIDATION_COMMIT"
