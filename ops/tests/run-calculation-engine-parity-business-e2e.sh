...
STEPS=(CEP_BASELINE CEP_COMPARE CEP_GATE)
PROJECT_ID="QA-CEP-$(date +%s)-$RANDOM"
EVIDENCE="$ROOT/var/test-evidence/calculation-engine-parity-latest.json"
...
carbonet_postgres_query "insert into framework_account_actor_assignment(account_id,tenant_id,project_id,actor_code,data_scope,assignment_status)
  values ('qaverify26','TEST_COMPANY_001','$PROJECT_ID','VERIFIER','$PROJECT_ID','ACTIVE'),
         ('qacalc26','TEST_COMPANY_001','$PROJECT_ID','CALCULATOR','$PROJECT_ID','ACTIVE')
...
[[ "$RUNTIME_COMMIT" =~ ^[0-9a-f]{40}$ ]] || { echo "CEP_E2E_RUNTIME_IDENTITY_INVALID" >&2; exit 3; }
CARBONET_RELAY_QA_PROJECT_ID="$PROJECT_ID" \
CARBONET_RELAY_PROCESS_CODE="$PROCESS" \
CARBONET_RELAY_STEPS="CEP_BASELINE,CEP_COMPARE,CEP_GATE" \
CARBONET_RELAY_STEP_ACTORS="VERIFIER,CALCULATOR,VERIFIER" \
CARBONET_RELAY_ACCOUNTS_JSON='{"VERIFIER":"qaverify26","CALCULATOR":"qacalc26"}' \
CARBONET_RELAY_ROUTE="/work/calculation-engine-parity" \
...
[[ "$residue" == 0 ]] || { echo "CEP_E2E_CLEANUP_FAILED residue=$residue" >&2; exit 3; }
