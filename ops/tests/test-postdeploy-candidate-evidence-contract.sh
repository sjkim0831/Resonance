...
assert 'candidateMode ? "7000" : "2500"' in screen_mjs
assert 'CARBONET_SCREEN_CONTRACT_SAVE_MAX_MS || defaultMaxSaveMillis' in screen_mjs
assert 'saved.elapsedMs <= maxSaveMillis' in screen_mjs
assert 2000 < int(re.search(r'candidateMode \? "[0-9]+" : "([0-9]+)"', screen_mjs).group(1)) < 3000
assert int(re.search(r'candidateMode \? "([0-9]+)" : "[0-9]+"', screen_mjs).group(1)) == 7000
...
if os.environ.get("CANDIDATE_EVIDENCE_SKIP_DEPLOY_WIRING") != "true":
    ...
    frontend_fast = deploy[deploy.index('if frontend_only_fast_path_eligible; then'):
                           deploy.index('# A measured JVM profile changes only the Deployment environment.')]
    ...
    assert 'CARBONET_QA_AUTH_LOCK_FILE="${CARBONET_USAGE_LEDGER_AUTH_LOCK_FILE:-/tmp/carbonet-qa-auth-usage-ledger-session.lock}"' in frontend_fast
