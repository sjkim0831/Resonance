...
for (const token of ["/admin/login/actionLogin", "adminBrowser: 1", "adminResponsive: 1", "adminAccessibility: 1", "pathname.startsWith(\"/admin/\")", "Math.max(Number(evidence.performanceP95Ms || 0), adminP95)", 'querySelectorAll("input,select,textarea,button,a[href]").length >= 4']) if (!engine.includes(token)) throw new Error(`admin engine contract missing: ${token}`);
