...
for token in validate-regulatory-submission-workflow.sh run-regulatory-submission-business-e2e.sh resonance-regulatory-admin-e2e.sh validate-customer-work-journey.sh framework_simulation_run "job_status='VERIFIED'" 'jobs<>58' 'tests<3'; do grep -Fq "$token" "$TARGET" || { echo "[regulatory-submission-contract] FAIL missing=$token" >&2; exit 1; }; done
