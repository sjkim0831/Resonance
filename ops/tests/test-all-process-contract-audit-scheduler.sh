...
grep -Fq 'WorkingDirectory=/opt/Resonance/var/deploy-worktrees/runtime-build' "$SERVICE" || fail 'service must execute from the current deploy worktree'
grep -Fq 'Environment=RESONANCE_ROOT=/opt/Resonance/var/deploy-worktrees/runtime-build' "$SERVICE" || fail 'service must pass the current deploy worktree as the canonical root'
! grep -Fxq 'WorkingDirectory=/opt/Resonance' "$SERVICE" || fail 'service must not execute the stale repository checkout'
...
grep -Fq 'RESONANCE_ROOT:-/opt/Resonance/var/deploy-worktrees/runtime-build' "$RUNNER" || fail 'runner default root must be the current deploy worktree'
! grep -Fq 'RESONANCE_ROOT:-/opt/Resonance}' "$RUNNER" || fail 'runner must not fall back to the stale repository checkout'
