-- AI Platform Management Schema - Migration v2
-- RAG Management, Training Candidates, Trace Tracking
-- Ensures all AI platform tables and columns exist
-- Target DB: CUBRID-compatible SQL
-- Apply: csql -u dba carbonet -i /tmp/20260613_ai_platform_migration.sql

-- ============================================
-- ACCESS_EVENT Compatibility Patch
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_ai_rag_doc_project_status
  ON ai_rag_document (project_id, status, frst_regist_pnttm);
CREATE INDEX IF NOT EXISTS idx_ai_rag_doc_source
  ON ai_rag_document (source_type, status);
CREATE INDEX IF NOT EXISTS idx_ai_rag_doc_parent
...
CREATE INDEX IF NOT EXISTS idx_ai_rag_chunk_document
  ON ai_rag_chunk (document_id, chunk_index);
CREATE INDEX IF NOT EXISTS idx_ai_rag_chunk_project_status
  ON ai_rag_chunk (project_id, status, frst_regist_pnttm);
CREATE INDEX IF NOT EXISTS idx_ai_rag_chunk_duplicate
  ON ai_rag_chunk (duplicate_group_id);
CREATE INDEX IF NOT EXISTS idx_ai_rag_chunk_embedded
...
CREATE INDEX IF NOT EXISTS idx_ai_vectordb_project_status
...
CREATE INDEX IF NOT EXISTS idx_ai_search_verif_project_time
  ON ai_search_verification (project_id, frst_regist_pnttm DESC);
CREATE INDEX IF NOT EXISTS idx_ai_search_verif_passed
...
CREATE INDEX IF NOT EXISTS idx_ai_training_candidate_project_status
  ON ai_training_candidate (project_id, status, frst_regist_pnttm DESC);
CREATE INDEX IF NOT EXISTS idx_ai_training_candidate_category
  ON ai_training_candidate (category, status, frst_regist_pnttm DESC);
CREATE INDEX IF NOT EXISTS idx_ai_training_candidate_review
  ON ai_training_candidate (review_status, status, frst_regist_pnttm DESC);
CREATE INDEX IF NOT EXISTS idx_ai_training_candidate_source
...
CREATE INDEX IF NOT EXISTS idx_ai_training_dataset_project_status
...
CREATE INDEX IF NOT EXISTS idx_ai_trace_project_time
  ON ai_trace_detail (project_id, frst_regist_pnttm DESC);
CREATE INDEX IF NOT EXISTS idx_ai_trace_status
  ON ai_trace_detail (status, frst_regist_pnttm DESC);
CREATE INDEX IF NOT EXISTS idx_ai_trace_hermes
...
CREATE INDEX IF NOT EXISTS idx_ai_token_usage_date_model
  ON ai_token_usage (period_date, model_name);
CREATE INDEX IF NOT EXISTS idx_ai_token_usage_project_date
...
CREATE INDEX IF NOT EXISTS idx_ai_prompt_project_type
...
INSERT INTO ai_rag_document (document_id, document_name, document_type, source_type, status, chunk_count, duplicate_rate, indexed_at)
SELECT 'DOC-005', 'Kubernetes 운영 가이드', 'OPS_GUIDE', 'PDF', 'ACTIVE', 428, 0.09, CURRENT_DATETIME
FROM db_root WHERE NOT EXISTS (SELECT 1 FROM ai_rag_document WHERE document_id = 'DOC-005');
...
INSERT INTO ai_training_candidate (candidate_id, category, source_type, title, description, quality_score, ai_classification, auto_class_confidence, status, review_status)
SELECT 'TC-006', 'DB_SCHEMA', 'HERMES_AUTO', 'CUBRID 테이블 설계 패턴', 'Carbonet 기본 테이블 설계 및 인덱스 전략', 0.91, 'DB_SCHEMA', 0.94, 'PENDING', 'AWAITING_REVIEW'
FROM db_root WHERE NOT EXISTS (SELECT 1 FROM ai_training_candidate WHERE candidate_id = 'TC-006');
...
INSERT INTO ai_vectordb_index (index_id, index_name, index_type, dimension, total_chunks, total_documents, index_size_bytes, status, last_rebuilt_at)
SELECT 'IDX-003', 'carbonet-documents', 'chroma', 1536, 12000, 456, 157286400, 'HEALTHY', CURRENT_DATETIME
FROM db_root WHERE NOT EXISTS (SELECT 1 FROM ai_vectordb_index WHERE index_id = 'IDX-003');
...
INSERT INTO ai_trace_detail (trace_id, user_query, total_duration_ms, total_tokens, prompt_tokens, completion_tokens, status, model_used, rag_retrieval_ms, rag_chunks_retrieved, tool_calls_count, stages_json)
SELECT 'TRACE-004',
       'Kubernetes 배포 자동화 방법을 알려주세요',
       2340, 678, 512, 166, 'SUCCESS', 'qwen2.5-coder-14b',
       145, 10, 4,
       '[{"stage":"RAG_SEARCH","duration_ms":145,"status":"SUCCESS"},{"stage":"MODEL_INFER","duration_ms":2050,"status":"SUCCESS"},{"stage":"TOOL_CALL","duration_ms":145,"status":"SUCCESS"}]'
FROM db_root WHERE NOT EXISTS (SELECT 1 FROM ai_trace_detail WHERE trace_id = 'TRACE-004');
...
INSERT INTO ai_search_verification (verification_id, query_text, top_k, returned_chunks, avg_relevance_score, passed_yn, verified_at)
SELECT 'VER-003', 'Kubernetes Pod 관리 방법', 10, 9, 0.88, 'Y', CURRENT_DATETIME
FROM db_root WHERE NOT EXISTS (SELECT 1 FROM ai_search_verification WHERE verification_id = 'VER-003');
...
INSERT INTO ai_token_usage (usage_id, period_date, model_name, prompt_tokens, completion_tokens, total_tokens, estimated_cost, request_count)
SELECT 'USAGE-003', CURRENT_DATE, 'supergemma-2.6b', 15000, 4500, 19500, 0.45, 220
FROM db_root WHERE NOT EXISTS (SELECT 1 FROM ai_token_usage WHERE usage_id = 'USAGE-003');

-- Sample Prompt Templates
INSERT INTO ai_prompt_template (prompt_id, prompt_name, prompt_type, version, system_prompt, model_name, temperature, status, active_yn)
SELECT 'PROMPT-001', 'Carbonet 코드 생성 프롬프트', 'CODE_GENERATION', '1.0',
       'Carbonet 플랫폼을 위한 코드를 생성합니다. 표준프레임워크 규칙을 준수하고, 보안 취약점이 없어야 합니다.',
       'qwen2.5-coder-7b', 0.3, 'ACTIVE', 'Y'
FROM db_root WHERE NOT EXISTS (SELECT 1 FROM ai_prompt_template WHERE prompt_id = 'PROMPT-001');

INSERT INTO ai_prompt_template (prompt_id, prompt_name, prompt_type, version, system_prompt, model_name, temperature, status, active_yn)
SELECT 'PROMPT-002', 'Carbonet 문서 요약 프롬프트', 'SUMMARIZATION', '1.0',
       'Carbonet 관련 문서를简洁하게 요약합니다. 핵심 내용과 대체 가능한 대안을 포함합니다.',
       'qwen2.5-coder-14b', 0.5, 'ACTIVE', 'Y'
FROM db_root WHERE NOT EXISTS (SELECT 1 FROM ai_prompt_template WHERE prompt_id = 'PROMPT-002');
