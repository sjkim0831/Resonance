...
PROCESS=EMISSION_CALCULATION
STEPS=(EMISSION_CALCULATION_01_PLAN EMISSION_CALCULATION_02_WORK EMISSION_CALCULATION_03_VERIFY EMISSION_CALCULATION_04_APPROVE)
CONTRACTS='[]'
...
[[ "$(tr -d '[:space:]' < "${CARBONET_DEPLOY_STATE_FILE:-/opt/resonance-data/deploy/carbonet-main-success.commit}")" == "$VALIDATION_COMMIT" ]] || { echo '[emission-calculation-assurance] FAIL validation commit is not deployed' >&2; exit 3; }
if [[ "$SOURCE_COMMIT" != "$VALIDATION_COMMIT" ]]; then
  ...
  for key in PLAN_RUNTIME_REQUIRED PLAN_FRONTEND_REQUIRED PLAN_BACKEND_REQUIRED PLAN_DATABASE_REQUIRED; do [[ "$(awk -F= -v key="$key" '$1==key{print $2}' <<<"$PLAN")" == false ]] || { echo "[emission-calculation-assurance] FAIL unreleased runtime change key=$key" >&2; exit 3; }; done
fi
RUNTIME="$(bash "$ROOT/ops/scripts/validate-emission-calculation-runtime.sh")"
grep -Eq '^\[calculation-runtime\] PASS .*formula=reconciled ' <<<"$RUNTIME" || exit 1
EVIDENCE="$(jq -cn --argjson contracts "$CONTRACTS" --arg runtime "$RUNTIME" --arg validationCommit "$VALIDATION_COMMIT" '{suite:"EMISSION_CALCULATION_ASSURANCE",validationCommit:$validationCommit,contracts:$contracts,runtime:$runtime,steps:4,caseFamilies:5,formulaReconciled:true}')"
...
carbonet_postgres_query "DO \$\$ DECLARE actual integer; BEGIN
 ...
 SELECT p.process_code,s.step_code,'PASSED',NULL,e.body||jsonb_build_object('stepCode',s.step_code),'EMISSION_CALCULATION_ASSURANCE',current_timestamp,'BUSINESS_E2E',p.process_version,'$SOURCE_COMMIT',framework_current_process_step_contract_fingerprint(p.process_code,s.step_code),'carbonet-prod','inline://business-e2e/sha256/$SHA','$SHA'
 ...
 SELECT count(*) INTO actual FROM framework_current_business_e2e_evidence WHERE process_code='$PROCESS' AND business_test_result='PASSED' AND source_commit='$SOURCE_COMMIT' AND evidence_hash='$SHA';
 IF actual<>4 THEN RAISE EXCEPTION 'emission calculation evidence mismatch: %',actual; END IF; END \$\$;" >/dev/null
printf '%s\n' "$RUNTIME"; printf '[emission-calculation-assurance] PASS current=4/4 cases=5 formula=reconciled sha256=%s\n' "$SHA"
