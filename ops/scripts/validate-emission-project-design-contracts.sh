  ...
  kubectl -n "$NAMESPACE" exec "$leader" -c "$CONTAINER" -- \
    psql -h 127.0.0.1 -U "$USER_NAME" -d "$DATABASE" -AtF' ' -qc "
      select
        ...
        (select count(distinct case_type) from framework_process_professional_scenario where process_code='EMISSION_PROJECT' and design_version=1 and case_type in ('CONCURRENCY','IDEMPOTENCY','DEADLINE','INTEGRATION','FILE_SECURITY','DATA_QUALITY','SEGREGATION','REPORT_INTEGRITY')),
        (select count(*) from framework_process_professional_scenario where process_code='EMISSION_PROJECT' and design_version=1 and case_type in ('CONCURRENCY','IDEMPOTENCY','DEADLINE','INTEGRATION','FILE_SECURITY','DATA_QUALITY','SEGREGATION','REPORT_INTEGRITY') and automated and expected_duration_minutes between 1 and 30),
