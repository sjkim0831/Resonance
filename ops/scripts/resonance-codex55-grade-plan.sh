...
echo "[DEPRECATED] resonance-codex55-grade-plan.sh: CUBRID는 제거됨. 이 스크립트는 더 이상 사용되지 않습니다."
exit 1
...
DB_USER="${DB_USER:-dba}"
OUT_DIR="${OUT_DIR:-$ROOT_DIR/var/ai-runtime/codex55-grade}"
...
TASK_ID="codex55-$STAMP-$$"
TRACE_ID="trace-codex55-$STAMP-$$"
...
EVENT_JSONL="$OUT_DIR/codex55-grade-events.jsonl"
...
context = {
    ...
    "codebase": {
        ...
        "agentArchitectureDocs": read(root_path / "docs/ai/60-operations/dev-orchestration/codex55-grade-agent-architecture.md", 5000),
    },
...
plan = {
    "taskType": "general",
    "riskLevel": "LOW",
    "interpretedIntent": "Hermes/Codex55 실행지능 경로가 응답 가능한지 확인하는 짧은 인사 또는 테스트 요청입니다.",
    "inferredMissingDetails": ["구체적인 수정 대상, 경로, 검증 조건은 아직 주어지지 않았습니다."],
    "affectedSurfaces": ["ai", "logs"],
    "targetRoute": "",
    "targetModule": "Hermes/Codex55-grade planner",
    ...
    "nextRecommendations": [
        {
            ...
            "command": "bash ops/scripts/resonance-codex55-grade-plan.sh \"수정할 기능과 검증 URL\"",
            "targetRoute": "",
            "targetModule": "Hermes/Codex55-grade planner",
...
system = """You are the Codex-5.5-grade execution intelligence layer for Carbonet/Resonance.
Use Korean. Return strict JSON only.
Your job is not to hallucinate execution. Your job is to infer the user's real target from request + system context + previous work, then produce a high-precision execution plan for Codex/scripts.
...
sql.append(
    ...
    + ", ".join([
        ...
        lit(plan.get("taskType") or "general", 80), lit(plan.get("riskLevel") or "MEDIUM", 30), "'READY_FOR_CODEX'",
        "'qwen3.6-40b-deck-opus-q4'", "'CODEX_55_GRADE_PLAN'", lit(plan.get("targetRoute"), 500),
        lit(plan.get("targetModule"), 500), "'carbonet'", lit(plan.get("planSummary"), 3900), lit(plan_json, 1000), "'resonance-codex55-grade'"
    ])
    + ");"
)
sql.append(
...
for idx, step in enumerate(steps, start=1):
    sql.append(
        ...
        + ", ".join([
            ...
            dump(step), lit(step.get("evidence"), 3900), lit(step.get("ownerRole") or "CODEX", 80), "'QWEN40_PRIMARY_PLANNER'", "'PENDING'"
        ])
        + ");"
    )
...
sql.append(
    ...
    + ", ".join([
        lit("decision-" + task_id, 80), lit(task_id, 80), "'CODEX55_GRADE_PLAN'", "'qwen3.6-40b-deck-opus-q4'",
        "'codex-5.5'", lit("40B 단일 모델을 Codex급 실행지능 레이어로 사용하되, 실행은 Codex/결정론 스크립트가 수행하도록 분리합니다."),
...
event = {
    ...
    "script": "resonance-codex55-grade-plan",
    "taskId": task_id,
