...
payload="$(jq -n --arg model "$selector_model" --arg prompt "$selector_prompt" '{model:$model,temperature:0,max_tokens:8,messages:[{role:"user",content:$prompt}]}')"
selection_raw="$(curl -fsS --max-time 30 "$SELECTOR_URL/chat/completions" -H 'Content-Type: application/json' -d "$payload")"
