...
db_sql "
  update resonance_projects__task
   ...
   where task_type='DESIGN_ASSET_PROMOTION' and status='RUNNING'
...
CLAIM="$(db_sql "
  with next_task as (
    select task_id
     ...
     where task_type='DESIGN_ASSET_PROMOTION' and status in ('PLANNED','RETRY')
     order by task_id
...
RESULT="$(jq -nc \
  ...
  '{validation:"PASS",delivery:"READY_FOR_RUNTIME_APPLIER",queueFile:$queueFile,artifactSha256:$artifactSha256}')"
