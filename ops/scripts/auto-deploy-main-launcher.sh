...
snapshot_legacy_automation_retirement_helper="$snapshot_dir/retire-legacy-runtime-mutation-automation.sh"
...
if ! git -C "$ROOT_DIR" show --format= --no-textconv \
  "$target_commit:ops/scripts/retire-legacy-runtime-mutation-automation.sh" \
  >"$snapshot_legacy_automation_retirement_helper"; then
  echo '[auto-deploy-launcher] target legacy-automation retirement helper is missing' >&2
  exit 79
fi
[[ -s "$snapshot_legacy_automation_retirement_helper" ]] || {
  echo '[auto-deploy-launcher] target legacy-automation retirement helper is empty' >&2
  exit 79
}
if ! git -C "$ROOT_DIR" show --format= --no-textconv \
...
snapshot_legacy_automation_retirement_helper_sha256="$(sha256sum "$snapshot_legacy_automation_retirement_helper" | awk '{print $1}')"
[[ "$snapshot_legacy_automation_retirement_helper_sha256" =~ ^[0-9a-f]{64}$ ]] || exit 79
...
[[ "$(stat -c '%a:%u' "$snapshot_legacy_automation_retirement_helper" 2>/dev/null || true)" \
   == "700:$(id -u)" \
   && "$(sha256sum "$snapshot_legacy_automation_retirement_helper" | awk '{print $1}')" \
      == "$snapshot_legacy_automation_retirement_helper_sha256" ]] || {
  echo '[auto-deploy-launcher] target legacy-automation retirement helper changed or is not private' >&2
  exit 79
}
