...
if [[ -z "$paths" ]]; then
  ...
  echo menu
elif ! grep -Evi '/[^/]*(actor|process|governance|delivery|workflow|topology|portfolio|handoff|notification|assignment|assignee|emission_site|onboarding|gnb|component|design|theme|section|page|screen|builder|asset|contract|schema_set|schema_contract|professional)[^/]*\.sql$' <<<"$paths" | grep -q .; then
  echo governance
