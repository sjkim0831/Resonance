...
for path in "${pages[@]}"; do code="$(curl -sS -L -b "$COOKIE_JAR" -o "$PAGE_BODY" -w '%{http_code}' "$BASE_URL$path")"; [[ "$code" == 200 ]] || { echo "[calculation-runtime] FAIL page=$path status=$code" >&2; exit 1; }; grep -qi '<!doctype html' "$PAGE_BODY" || exit 1; done
  for _ in $(seq 1 20); do read -r sample_status sample_time <<<"$(curl -sS -b "$COOKIE_JAR" -o /dev/null -w '%{http_code} %{time_total}' "$BASE_URL/home/api/emission-projects/$project_id/calculation")"; [[ "$sample_status" == 200 ]] || { echo "[calculation-runtime] FAIL p95 probe status=$sample_status" >&2; exit 1; }; printf '%s\n' "$sample_time" >>"$TIMINGS"; done
