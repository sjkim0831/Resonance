...
baseline_ms="$(
  if [[ -s "$history_file" ]]; then
    jq -rs --arg mode "$mode" '
      [ .[] | select(.mode == $mode and .status == "PASS") | .elapsedMs ]
      | .[-10:]
...
elif (( elapsed_ms > regression_limit_ms )); then
  status="SLO_BREACH"
  reason="TARGET_OR_25_PERCENT_REGRESSION"
fi
...
record="$(
  jq -cn \
    ...
    '{
      ...
      reason:$reason,elapsedMs:$elapsedMs,targetMs:$targetMs,
...
EOF
  ...
  echo "[deploy-performance] WARN mode=$mode elapsed=${elapsed_ms}ms target=${target_ms}ms baseline=${baseline_ms}ms reason=$reason" >&2
else
  rm -f "$diagnostic_file"
  echo "[deploy-performance] PASS mode=$mode elapsed=${elapsed_ms}ms target=${target_ms}ms baseline=${baseline_ms}ms"
fi
