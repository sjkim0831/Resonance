    ...
    psql -v ON_ERROR_STOP=1 -h 127.0.0.1 -U postgres -d carbonet -AtF '|' -c "
      select e.emplyr_id,
        ...
        join framework_identity_sync_audit sync
          on lower(sync.account_id)=lower(e.emplyr_id)
       where lower(trim(e.emplyr_id))=lower('$USERNAME')
       group by e.emplyr_id,s.author_code"
)"
IFS='|' read -r user_id author_code actor_count result_code password_matches <<<"$result"
echo "[identity-sync-verify] observed username=${user_id:-missing} author=${author_code:-missing} actors=${actor_count:-0} audit=${result_code:-missing} password=${password_matches:-missing}"
[[ "${user_id,,}" == "${USERNAME,,}" ]]
