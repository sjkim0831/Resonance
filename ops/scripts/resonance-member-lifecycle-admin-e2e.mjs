...
for (const route of routes) {
  ...
  if (response.status() !== 200 || !route.validate(pagePayload)) {
    const shape = Object.fromEntries(Object.entries(pagePayload || {}).map(([key, value]) => [key,
      typeof value === "boolean" ? value : Array.isArray(value) ? `array:${value.length}` : typeof value]));
