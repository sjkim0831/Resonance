...
KEYCLOAK_MANIFEST="$ROOT/deploy/k8s/control-plane/keycloak.yaml"
...
validate() {
  ...
  kubectl apply --dry-run=client -f "$KEYCLOAK_MANIFEST" >/dev/null
  if command -v ruby >/dev/null 2>&1; then
    ...
    ruby -e 'require "yaml"; ARGV.each { |path| YAML.load_stream(File.read(path), aliases: true) }' \
      ...
      "$BACKSTAGE_APP/app-config.oidc.yaml"
  fi
