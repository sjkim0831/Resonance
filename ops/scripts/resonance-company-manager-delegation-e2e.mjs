...
try {
  ...
  const browser=await chromium.launch({headless:true}); const routes=[]; const samples=[];
  try {
    ...
    for(const actor of actors)for(const viewport of viewports){
      ...
      await page.goto(`${base}${actor.prefix}/company-manager-delegation?projectId=${encodeURIComponent(projectId)}`,{waitUntil:"domcontentloaded",timeout:20000});await page.getByRole("heading",{name:"회원사 관리자 위임·승계·업무 인계"}).waitFor({timeout:12000});errors.length=0;
      sessions.push({actor,viewport,context,page,errors});
    }
    for(const session of sessions)for(const step of steps){
      const response=await session.page.goto(`${base}${session.actor.prefix}/company-manager-delegation?step=${step}&projectId=${encodeURIComponent(projectId)}`,{waitUntil:"domcontentloaded",timeout:20000});await session.page.getByRole("heading",{name:"회원사 관리자 위임·승계·업무 인계"}).waitFor({timeout:12000});
