...
function waitForRouteData(page) { return page.locator("select option").first().waitFor({state:"attached",timeout:12000}); }
...
try {
  ...
  try {
    ...
    for(const actor of actors)for(const viewport of viewports){
      ...
      const settled=waitForRouteData(page);await page.goto(`${base}${actor.prefix}/company-manager-delegation?projectId=${encodeURIComponent(projectId)}`,{waitUntil:"domcontentloaded",timeout:20000});await page.getByRole("heading",{name:"회원사 관리자 위임·승계·업무 인계"}).first().waitFor({timeout:12000});await settled;errors.length=0;
      sessions.push({actor,viewport,context,page,errors});
    }
    for(const session of sessions)for(const step of steps){
      await session.page.bringToFront();
      const settled=waitForRouteData(session.page);const response=await session.page.goto(`${base}${session.actor.prefix}/company-manager-delegation?step=${step}&projectId=${encodeURIComponent(projectId)}`,{waitUntil:"domcontentloaded",timeout:20000});await session.page.getByRole("heading",{name:"회원사 관리자 위임·승계·업무 인계"}).first().waitFor({timeout:12000});await settled;
      ...
      if(response?.status()!==200||session.errors.length||measured.overflow||measured.unnamed)throw new Error(`UI_CONTRACT_${session.actor.audience}_${step}_${session.viewport.name}`);routes.push({audience:session.actor.audience,step,viewport:session.viewport.name,status:200,duration:measured.duration,resources:measured.resources});
    }
    for(let index=0;index<20;index++){const session=sessions[index%sessions.length];await session.page.bringToFront();const step=steps[index%steps.length];const duration=await session.page.evaluate(async ({step,index})=>{const started=performance.now(),url=new URL(location.href);url.searchParams.set("step",step);url.searchParams.set("sample",String(index));history.pushState({},"",url);window.dispatchEvent(new Event("carbonet:navigate"));await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));return Math.round(performance.now()-started);},{step,index});await session.page.getByRole("heading",{name:"회원사 관리자 위임·승계·업무 인계"}).first().waitFor({timeout:12000});await waitForRouteData(session.page);if(new URL(session.page.url()).searchParams.get("step")!==step)throw new Error(`SPA_ROUTE_STATE_${step}`);samples.push(duration);sampleDetails.push({audience:session.actor.audience,viewport:session.viewport.name,step,duration});}
    await Promise.all(sessions.map(session=>session.context.close()));
  } finally { await browser.close(); }
  const sorted=[...samples].sort((a,b)=>a-b),p95=sorted[Math.ceil(sorted.length*.95)-1],hardRouteDurations=routes.map(route=>route.duration).sort((a,b)=>a-b),hardRouteP95Ms=hardRouteDurations[Math.ceil(hardRouteDurations.length*.95)-1];
  await call(successor,"DELETE",`/home/api/emission-projects/${encodeURIComponent(projectId)}`); deleted=true;
  console.log(JSON.stringify({status:"PASS",promotionEligible:false,processCode:"COMPANY_MANAGER_DELEGATION",stepCount:3,caseCount:5,request:1,idempotency:1,authorityDenial:1,approval:1,atomicHandover:1,successorVisible:1,projectCleanup:1,cleanup:0,api:1,database:1,authority:1,responsive:1,accessibility:1,exceptionStates:1,audit:1,recovery:1,desktop:1,mobile:1,routes,sampleDetails,performanceMeasurementMode:"SPA_WORKFLOW_STEP_NAVIGATION",performanceSampleCount:samples.length,performanceSamples:samples,performanceP95Ms:p95,hardRouteSampleCount:hardRouteDurations.length,hardRouteP95Ms,steps:{CMD_REQUEST:{result:"PASSED"},CMD_APPROVE:{result:"PASSED"},CMD_HANDOVER:{result:"PASSED"}},cases:{COMPANY_MANAGER_DELEGATION_AUTHORITY:{result:"PASSED"},COMPANY_MANAGER_DELEGATION_EXCEPTION:{result:"PASSED"},COMPANY_MANAGER_DELEGATION_HAPPY:{result:"PASSED"},COMPANY_MANAGER_DELEGATION_ISOLATION:{result:"PASSED"},COMPANY_MANAGER_DELEGATION_RECOVERY:{result:"PASSED"}}}));
} finally {
