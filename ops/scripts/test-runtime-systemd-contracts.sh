...
grep -Fq 'validate service $WEB_SERVICE without mutating deployment or service policy' "$root/ops/scripts/resonance-up.sh"
! grep -Fq 'scale "deployment/$WEB_DEPLOYMENT"' "$root/ops/scripts/resonance-up.sh"
! grep -Fq 'delete service "$WEB_SERVICE"' "$root/ops/scripts/resonance-up.sh"
! grep -Fq 'delete endpoints "$WEB_SERVICE"' "$root/ops/scripts/resonance-up.sh"
