...
LOCK_FILE="$RUN_DIR/resonance-k8s-build-deploy-20080.lock"
...
log_event() {
  ...
  local code="$2"
  local message="$3"
  printf '{"ts":"%s","script":"resonance-k8s-build-deploy-20080","status":"%s","code":"%s","image":"%s","message":"%s"}\n' \
...
timeline_event() {
  ...
  printf '{"ts":"%s","script":"resonance-k8s-build-deploy-20080","phase":"%s","image":"%s","detail":"%s","pods":"%s"}\n' \
