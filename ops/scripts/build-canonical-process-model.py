...
def main():
 ...
 for cap in orphan_caps:
  ...
  target['steps'].append({'stepCode':cap['menuCode'],'stepOrder':0,'stepName':cap['name'],'actorCode':binding.get('actor_code') or 'PLATFORM_OPERATOR','fromState':'','toState':'','commandCode':f"OPEN_{cap['menuCode']}",'screenCode':bp.get('page_id') or route or 'UNRESOLVED','apiCode':binding.get('api_contract') or 'UNRESOLVED','implementationStatus':'VERIFIED' if cap.get('routeTrace',{}).get('routeMatch')=='EXACT' else 'PARTIAL','implementationEvidence':{'menuCode':cap['menuCode'],'routeMatch':cap.get('routeTrace',{}).get('routeMatch','NONE')},'completionRules':['routeResolved','businessActionVerified','auditEvidenceProduced'],'failureTransitions':['ROUTE_MISSING','AUTHORITY_DENIED'],'rollbackCommand':f"RETURN_{cap['menuCode']}"});normalize_steps(target)
