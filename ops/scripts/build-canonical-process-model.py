...
def asset_catalog_processes(unregistered):
 labels={'frontendScreens':'프론트 화면·공통 UI','javaEndpoints':'Java API 엔드포인트','databaseObjects':'DB 객체','testSources':'자동 테스트 소스'}
 prefixes={'frontendScreens':'FRONTEND','javaEndpoints':'API','databaseObjects':'DATABASE','testSources':'TEST'}
 result=[]
 for kind in ('frontendScreens','javaEndpoints','databaseObjects','testSources'):
  assets=sorted(set(unregistered.get(kind,[])))
  for chunk_index,chunk in enumerate(balanced_chunks(assets),1):
   steps=[]
   for order,asset in enumerate(chunk,1):
    identity=hashlib.sha256(f'{kind}:{asset}'.encode()).hexdigest()[:16].upper();code=f'ASSET_{prefixes[kind]}_{identity}'
    api=asset.split(':',2)[-1] if kind=='javaEndpoints' else 'UNRESOLVED'
    screen=asset if kind in {'frontendScreens','testSources'} else 'UNRESOLVED'
    steps.append({'stepCode':code,'stepOrder':order,'stepName':asset,'actorCode':'PLATFORM_OPERATOR','fromState':'READY' if order==1 else f'S{order-1:02d}_DONE','toState':f'S{order:02d}_DONE','commandCode':f'INSPECT_{code}','screenCode':screen,'apiCode':api,'implementationStatus':'VERIFIED','implementationEvidence':{'assetType':kind,'assetRef':asset,'coverage':{'screen':kind=='frontendScreens','api':kind=='javaEndpoints','database':kind=='databaseObjects','authority':False,'test':kind=='testSources'}},'completionRules':['assetExists','assetFingerprintStable','catalogTraceResolved'],'failureTransitions':['ASSET_MISSING','FINGERPRINT_CHANGED'],'rollbackCommand':'NOT_APPLICABLE_READ_ONLY'})
   process={'processCode':f'IMPL_ASSET_{prefixes[kind]}_{chunk_index:03d}','processName':f"기개발 {labels[kind]} 카탈로그 {chunk_index}",'domainCode':'IMPLEMENTED_TECHNICAL_ASSET','ownerActorCode':'PLATFORM_OPERATOR','participantActorCodes':[],'modelType':'IMPLEMENTED_ASSET_CATALOG','source':'CURRENT_SYSTEM_UNREGISTERED_ASSET_CATALOG','assetType':kind,'steps':steps}
   result.append(process)
 return result

def main():
 ap=argparse.ArgumentParser();ap.add_argument('--implemented',type=Path,default=OUT/'implemented-process-model.json');ap.add_argument('--evidence',type=Path,default=OUT/'implemented-evidence-graph.json');ap.add_argument('--design',type=Path,default=OUT/'processes.json');ap.add_argument('--design-actors',type=Path,default=OUT/'actors.json');ap.add_argument('--runtime-db',type=Path,default=OUT/'runtime-db-inventory.json');ap.add_argument('--design-tests',type=Path,default=OUT/'test-scenarios.jsonl');ap.add_argument('--out',type=Path,default=OUT);args=ap.parse_args()
 ...
 evidence_graph=json.loads(args.evidence.read_text(encoding='utf-8-sig')) if args.evidence.exists() else {'unregisteredImplementedAssets':{}}
 ...
 for group in implemented['subprocesses']:
  ...
  for i,code in enumerate(group['capabilityCodes'],1):
   cap=caps[code];binding=(cap.get('bindings') or [{}])[0];bp=(cap.get('screenBlueprints') or [{}])[0];actor=binding.get('actor_code') or ACTOR_BY_GROUP.get(group['menuCode']) or ACTOR_BY_ROOT.get(group['parentCode'],'PLATFORM_OPERATOR');route=cap.get('url') or '';step_evidence=compact_evidence(cap);status='VERIFIED' if step_evidence['coverage'].get('screen') and step_evidence['coverage'].get('api') else ('PARTIAL' if route or cap.get('bindings') or any(step_evidence['coverage'].values()) else 'UNRESOLVED')
   steps.append({'stepCode':cap['menuCode'],'stepOrder':i,'stepName':cap['name'],'actorCode':actor,'fromState':'READY' if i==1 else f'S{i-1:02d}_DONE','toState':f'S{i:02d}_DONE','commandCode':f"OPEN_{cap['menuCode']}",'screenCode':bp.get('page_id') or route or 'UNRESOLVED','apiCode':binding.get('api_contract') or (step_evidence['apiEndpoints'][0] if step_evidence['apiEndpoints'] else 'UNRESOLVED'),'implementationStatus':status,'implementationEvidence':step_evidence,'completionRules':['routeResolved','businessActionVerified','auditEvidenceProduced'],'failureTransitions':['ROUTE_MISSING','AUTHORITY_DENIED','CONTRACT_MISSING'],'rollbackCommand':f"RETURN_{cap['menuCode']}"})
 ...
 catalog_processes=asset_catalog_processes(evidence_graph.get('unregisteredImplementedAssets',{}));processes.extend(catalog_processes);all_steps.extend(s for p in catalog_processes for s in p['steps'])
 ...
 for cap in orphan_caps:
  ...
  step_evidence=compact_evidence(cap);target['steps'].append({'stepCode':cap['menuCode'],'stepOrder':0,'stepName':cap['name'],'actorCode':binding.get('actor_code') or 'PLATFORM_OPERATOR','fromState':'','toState':'','commandCode':f"OPEN_{cap['menuCode']}",'screenCode':bp.get('page_id') or route or 'UNRESOLVED','apiCode':binding.get('api_contract') or (step_evidence['apiEndpoints'][0] if step_evidence['apiEndpoints'] else 'UNRESOLVED'),'implementationStatus':'VERIFIED' if step_evidence['coverage'].get('screen') and step_evidence['coverage'].get('api') else 'PARTIAL','implementationEvidence':step_evidence,'completionRules':['routeResolved','businessActionVerified','auditEvidenceProduced'],'failureTransitions':['ROUTE_MISSING','AUTHORITY_DENIED'],'rollbackCommand':f"RETURN_{cap['menuCode']}"});normalize_steps(target)
 ...
 for process in catalog_processes:
  for step in process['steps']:
   canonical_tests.append({'caseCode':f"TC_{step['stepCode']}_CATALOG_TRACE",'processCode':process['processCode'],'stepCode':step['stepCode'],'caseType':'TRACEABILITY','variant':'SOURCE_EXISTS_AND_UNIQUE','severity':'MAJOR','given':['implementedAssetInventoryRefreshed'],'when':{'commandCode':step['commandCode'],'assetRef':step['implementationEvidence']['assetRef']},'then':['assetExists','exactlyOneCatalogStep','readOnlyLockPresent'],'status':'GENERATED_TRACEABILITY_GATE'})
 ...
 expected_assets={(kind,asset) for kind,assets in evidence_graph.get('unregisteredImplementedAssets',{}).items() for asset in assets};registered_assets={(s.get('implementationEvidence',{}).get('assetType'),s.get('implementationEvidence',{}).get('assetRef')) for p in catalog_processes for s in p['steps']};registered_assets.discard((None,None));missing_assets=sorted(expected_assets-registered_assets);duplicate_asset_refs=sorted(ref for ref in registered_assets if sum(1 for p in catalog_processes for s in p['steps'] if (s.get('implementationEvidence',{}).get('assetType'),s.get('implementationEvidence',{}).get('assetRef'))==ref)>1)
 ...
 stats={'canonicalProcesses':len(processes),'implementedProcesses':len(implemented['subprocesses']),'newRequiredProcesses':len(new_required),'canonicalSteps':len(all_steps),'canonicalScenarios':len(canonical_tests),'designProcessesReusedThroughExisting':len(mapped & design_codes),'designProcessesAddedAsNew':len(added_codes),'unmappedDesignProcesses':len(unmapped_not_added),'unresolvedMappingTargets':len(unresolved_targets),'duplicateProcessCodes':len(duplicate_codes),'emptyProcesses':len(empty),'processesBelowComplexityFloor':len(too_small),'processesAboveComplexityCeiling':len(too_large),'preservedImplementedCapabilities':len(implemented_capability_codes & canonical_implemented_codes),'registeredImplementedAssets':len(registered_assets),'unregisteredImplementedAssets':len(missing_assets),'implementedAssetCatalogProcesses':len(catalog_processes),'immutableImplementedProcesses':len(immutable_processes),'immutableImplementedSteps':sum(len(p['steps']) for p in immutable_processes),**evidence_counts}
 validation={'duplicateProcessCodes':duplicate_codes,'emptyProcesses':empty,'unmappedDesignProcesses':unmapped_not_added,'unresolvedMappingTargets':unresolved_targets,'processesBelowComplexityFloor':too_small,'processesAboveComplexityCeiling':too_large,'missingImplementedCapabilityCodes':missing_implemented,'missingImplementedAssets':missing_assets,'duplicateImplementedAssetRefs':duplicate_asset_refs,'unlockedImplementedProcessCodes':unlocked_implemented,'unlockedImplementedStepCodes':unlocked_implemented_steps}
