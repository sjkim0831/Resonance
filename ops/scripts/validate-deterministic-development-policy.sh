...
grep -Fq 'QUALITY_CONTRACT_V2_RETRY' "$ROOT/ops/scripts/run-project-auto-completion-orchestrator.sh"
grep -Fq 'COMPONENT_COMMON|CLASS_PROPERTY_COMMON|UI_QUALITY)' "$RUNNER"
grep -Fq 'UI_QUALITY|COMPONENT_COMMON|CLASS_PROPERTY_COMMON)' "$ROOT/ops/scripts/validate-generated-process-dimension.sh"
