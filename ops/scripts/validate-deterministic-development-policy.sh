...
jq -e '
  ...
  (.deterministicJobTypes | sort == ["API","API_QUALITY","BACKEND","BACKEND_QUALITY","DATABASE","DATABASE_QUALITY","DEPLOYMENT","DESIGN","DESIGN_PREFLIGHT","NOTIFICATION","REFERENCE_ANALYSIS"])
' "$POLICY" >/dev/null
bash -n "$WORKER"
bash -n "$RUNNER"
