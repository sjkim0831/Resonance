/**
 * Contract: #6 | Route: /admin/emission/survey-admin-data
 * Process: EMISSION_PROJECT | Actor: SITE_DATA_OWNER
 * Screen: 활동자료·증빙 수집 관리자 업무 화면
 * Generated: 2026-07-25T06:45:29.039182
 */
...
export const C6_활동자료증빙 수집 관리자 업무 화면Screen = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // POST: upload
      const r = await fetch('/home/api/emission-projects/{id}/activities/upload',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)});
      const d = await r.json();
      // GET: quality
      const r = await fetch('/home/api/emission-projects/{id}/quality');
      const d = await r.json(); setData(d);
      // POST: quality
      const r = await fetch('/home/api/emission-projects/{id}/quality',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)});
      const d = await r.json();
      // GET: submissions
      const r = await fetch('/home/api/emission-projects/{id}/submissions');
      const d = await r.json(); setData(d);
    } catch (err) {
  ...
  return (
    <Card>
      <CardHeader title="활동자료·증빙 수집 관리자 업무 화면" subheader="Contract-based" />
      <CardContent>
        ...
        {!error && screenState !== 'LOADING' && (
          <Box>
            <Typography variant="body2" color="textSecondary">
              Process: EMISSION_PROJECT | Actor: SITE_DATA_OWNER | Contract: #6
            </Typography>
            <Divider sx={ my: 2 } />
...
export default C6_활동자료증빙 수집 관리자 업무 화면Screen;
