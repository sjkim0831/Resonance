/**
 * Contract: #512 | Route: /admin/emission/organizational-boundary
 * Process: ORGANIZATIONAL_BOUNDARY | Actor: COMPANY_MANAGER
 * Screen: 경계 기준·포함 여부 판정 관리자 업무 화면
 * Generated: 2026-07-25T06:45:29.042366
 */
...
export const C512_경계 기준포함 여부 판정 관리자 업무 화면Screen = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      // POST: review-ready
      const r = await fetch('/home/api/emission-projects/{id}/organizational-boundary/review-ready',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)});
  ...
  return (
    <Card>
      <CardHeader title="경계 기준·포함 여부 판정 관리자 업무 화면" subheader="Contract-based" />
      <CardContent>
        ...
        {!error && screenState !== 'LOADING' && (
          <Box>
            <Typography variant="body2" color="textSecondary">
              Process: ORGANIZATIONAL_BOUNDARY | Actor: COMPANY_MANAGER | Contract: #512
            </Typography>
            <Divider sx={ my: 2 } />
...
export default C512_경계 기준포함 여부 판정 관리자 업무 화면Screen;
