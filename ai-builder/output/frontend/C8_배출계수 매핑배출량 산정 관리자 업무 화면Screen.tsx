/**
 * Contract: #8 | Route: /admin/emission/calculation-rule
 * Process: EMISSION_PROJECT | Actor: CALCULATOR
 * Screen: 배출계수 매핑·배출량 산정 관리자 업무 화면
 * Generated: 2026-07-25T06:45:29.039253
 */
...
export const C8_배출계수 매핑배출량 산정 관리자 업무 화면Screen = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      // GET: calculation
      const r = await fetch('/home/api/emission-projects/{id}/calculation');
      const d = await r.json(); setData(d);
      // POST: factor
      const r = await fetch('/home/api/emission-projects/{id}/activities/{activityId}/factor',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)});
      const d = await r.json();
      // POST: auto-map
      const r = await fetch('/home/api/emission-projects/{id}/activities/auto-map',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)});
      const d = await r.json();
      // POST: calculation
      const r = await fetch('/home/api/emission-projects/{id}/calculation',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)});
      const d = await r.json();
    } catch (err) {
  ...
  return (
    <Card>
      <CardHeader title="배출계수 매핑·배출량 산정 관리자 업무 화면" subheader="Contract-based" />
      <CardContent>
        ...
        {!error && screenState !== 'LOADING' && (
          <Box>
            <Typography variant="body2" color="textSecondary">
              Process: EMISSION_PROJECT | Actor: CALCULATOR | Contract: #8
            </Typography>
            <Divider sx={ my: 2 } />
...
export default C8_배출계수 매핑배출량 산정 관리자 업무 화면Screen;
