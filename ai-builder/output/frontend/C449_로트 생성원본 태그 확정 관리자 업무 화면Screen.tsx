/**
 * Contract: #449 | Route: /admin/work/co2-lot-tag-management?step=clt_create
 * Process: CO2_LOT_TAG_MANAGEMENT | Actor: TRADE_OPERATOR
 * Screen: 로트 생성·원본 태그 확정 관리자 업무 화면
 * Generated: 2026-07-25T06:45:29.041969
 */
...
export const C449_로트 생성원본 태그 확정 관리자 업무 화면Screen = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      // GET: clt_create
      const r = await fetch('/api/work/co2-lot-tag-management/clt_create');
  ...
  return (
    <Card>
      <CardHeader title="로트 생성·원본 태그 확정 관리자 업무 화면" subheader="Contract-based" />
      <CardContent>
        ...
        {!error && screenState !== 'LOADING' && (
          <Box>
            <Typography variant="body2" color="textSecondary">
              Process: CO2_LOT_TAG_MANAGEMENT | Actor: TRADE_OPERATOR | Contract: #449
            </Typography>
            <Divider sx={ my: 2 } />
...
export default C449_로트 생성원본 태그 확정 관리자 업무 화면Screen;
