/**
 * Contract: #338 | Route: /ccus/facility/facility-emergency-response?step=fer_recover
 * Process: FACILITY_EMERGENCY_RESPONSE | Actor: HSE_MANAGER
 * Screen: 원인분석·복구·재발방지 사용자 업무 화면
 * Generated: 2026-07-25T06:45:29.041909
 */
...
export const C338_원인분석복구재발방지 사용자 업무 화면Screen = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      // GET: fer_recover
      const r = await fetch('/api/ccus/facility/facility-emergency-response/fer_recover');
  ...
  return (
    <Card>
      <CardHeader title="원인분석·복구·재발방지 사용자 업무 화면" subheader="Contract-based" />
      <CardContent>
        ...
        {!error && screenState !== 'LOADING' && (
          <Box>
            <Typography variant="body2" color="textSecondary">
              Process: FACILITY_EMERGENCY_RESPONSE | Actor: HSE_MANAGER | Contract: #338
            </Typography>
            <Divider sx={ my: 2 } />
...
export default C338_원인분석복구재발방지 사용자 업무 화면Screen;
