/**
 * Contract: #239 | Route: /emission/my-tasks
 * Process: CUSTOMER_WORK_COORDINATION | Actor: COMPANY_MANAGER
 * Screen: 내 업무·프로젝트 선택
 * Generated: 2026-07-25T06:45:29.040227
 */
...
export const C239_내 업무프로젝트 선택Screen = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      // GET: emission-tasks
      const r = await fetch('/home/api/emission-tasks');
      const d = await r.json(); setData(d);
      // GET: completion
      const r = await fetch('/home/api/emission-projects/{id}/completion');
      const d = await r.json(); setData(d);
    } catch (err) {
  ...
  return (
    <Card>
      <CardHeader title="내 업무·프로젝트 선택" subheader="Contract-based" />
      <CardContent>
        ...
        {!error && screenState !== 'LOADING' && (
          <Box>
            <Typography variant="body2" color="textSecondary">
              Process: CUSTOMER_WORK_COORDINATION | Actor: COMPANY_MANAGER | Contract: #239
            </Typography>
            <Divider sx={ my: 2 } />
...
export default C239_내 업무프로젝트 선택Screen;
