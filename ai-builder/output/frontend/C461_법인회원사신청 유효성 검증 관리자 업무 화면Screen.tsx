/**
 * Contract: #461 | Route: /admin/work/certification-eligibility-check?step=cec_validate_company
 * Process: CERTIFICATION_ELIGIBILITY_CHECK | Actor: CERTIFICATE_OFFICER
 * Screen: 법인·회원사·신청 유효성 검증 관리자 업무 화면
 * Generated: 2026-07-25T06:45:29.042061
 */
...
export const C461_법인회원사신청 유효성 검증 관리자 업무 화면Screen = () => {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      // GET: cec_validate_company
      const r = await fetch('/api/work/certification-eligibility-check/cec_validate_company');
  ...
  return (
    <Card>
      <CardHeader title="법인·회원사·신청 유효성 검증 관리자 업무 화면" subheader="Contract-based" />
      <CardContent>
        ...
        {!error && screenState !== 'LOADING' && (
          <Box>
            <Typography variant="body2" color="textSecondary">
              Process: CERTIFICATION_ELIGIBILITY_CHECK | Actor: CERTIFICATE_OFFICER | Contract: #461
            </Typography>
            <Divider sx={ my: 2 } />
...
export default C461_법인회원사신청 유효성 검증 관리자 업무 화면Screen;
