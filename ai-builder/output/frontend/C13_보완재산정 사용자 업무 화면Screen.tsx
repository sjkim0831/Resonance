/**
 * Contract: #13 | Route: /emission/data_input?mode=correction
 * Process: EMISSION_PROJECT | Actor: SITE_DATA_OWNER
 * Screen: 보완·재산정 사용자 업무 화면
 * Generated: 2026-07-25T06:45:29.039427
 */
...
export const C13_보완재산정 사용자 업무 화면Screen = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const d = await r.json();
      // POST: submit
      const r = await fetch('/home/api/emission-projects/{id}/submissions/{submissionId}/submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)});
      const d = await r.json();
    } catch (err) {
  ...
  return (
    <Card>
      <CardHeader title="보완·재산정 사용자 업무 화면" subheader="Contract-based" />
      <CardContent>
        ...
        {!error && screenState !== 'LOADING' && (
          <Box>
            <Typography variant="body2" color="textSecondary">
              Process: EMISSION_PROJECT | Actor: SITE_DATA_OWNER | Contract: #13
            </Typography>
            <Divider sx={ my: 2 } />
...
export default C13_보완재산정 사용자 업무 화면Screen;
