/**
 * Contract: #3025 | Route: /admin/member/login_history
 * Process: ACCOUNT_LOCK_RECOVERY | Actor: APPROVER
 * Screen: 복구 승인·원자적 적용·통지 관리자 업무 화면
 * Generated: 2026-07-25T06:45:29.046296
 */
...
export const C3025_복구 승인원자적 적용통지 관리자 업무 화면Screen = () => {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      // POST: COMPLETE_ACCOUNT_RECOVERY
      const r = await fetch('/admin/api/member-recovery/{requestId}/complete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)});
  ...
  return (
    <Card>
      <CardHeader title="복구 승인·원자적 적용·통지 관리자 업무 화면" subheader="Contract-based" />
      <CardContent>
        ...
        {!error && screenState !== 'LOADING' && (
          <Box>
            <Typography variant="body2" color="textSecondary">
              Process: ACCOUNT_LOCK_RECOVERY | Actor: APPROVER | Contract: #3025
            </Typography>
            <Divider sx={ my: 2 } />
...
export default C3025_복구 승인원자적 적용통지 관리자 업무 화면Screen;
