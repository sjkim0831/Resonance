/**
 * Contract: #3131 | Route: /admin/generated/user-authority-assignment/user-authority-assignment-s4
 * Process: USER_AUTHORITY_ASSIGNMENT | Actor: APPROVER
 * Screen: 사용자별 권한 배정 - 승인·확정·통지·후속업무 연결 관리자 업무 화면
 * Generated: 2026-07-25T06:45:29.048614
 */
...
// USER_AUTHORITY_ASSIGNMENT_COMPLETE: POST /home/api/process-executions/{executionId}/commands
// LOAD_DRAFT: GET /home/api/process-executions/draft
// SAVE_DRAFT: PUT /home/api/process-executions/draft

export const C3131_사용자별 권한 배정  승인확정통지후속업무 연결 관리자 업무 화면Screen = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const d = await r.json(); setData(d);
      // POST: USER_AUTHORITY_ASSIGNMENT_COMPLETE
  ...
  return (
    <Card>
      <CardHeader title="사용자별 권한 배정 - 승인·확정·통지·후속업무 연결 관리자 업무 화면" subheader="Contract-based" />
      <CardContent>
        ...
        {!error && screenState !== 'LOADING' && (
          <Box>
            <Typography variant="body2" color="textSecondary">
              Process: USER_AUTHORITY_ASSIGNMENT | Actor: APPROVER | Contract: #3131
            </Typography>
            <Divider sx={ my: 2 } />
...
export default C3131_사용자별 권한 배정  승인확정통지후속업무 연결 관리자 업무 화면Screen;
