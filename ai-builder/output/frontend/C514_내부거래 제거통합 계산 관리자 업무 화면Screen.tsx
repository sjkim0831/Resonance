/**
 * Contract: #514 | Route: /admin/emission/organizational-boundary
 * Process: ORGANIZATIONAL_BOUNDARY | Actor: CALCULATOR
 * Screen: 내부거래 제거·통합 계산 관리자 업무 화면
 * Generated: 2026-07-25T06:45:29.042407
 */
...
export const C514_내부거래 제거통합 계산 관리자 업무 화면Screen = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      // POST: consolidate
      const r = await fetch('/home/api/emission-projects/{id}/organizational-boundary/consolidate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)});
  ...
  return (
    <Card>
      <CardHeader title="내부거래 제거·통합 계산 관리자 업무 화면" subheader="Contract-based" />
      <CardContent>
        ...
        {!error && screenState !== 'LOADING' && (
          <Box>
            <Typography variant="body2" color="textSecondary">
              Process: ORGANIZATIONAL_BOUNDARY | Actor: CALCULATOR | Contract: #514
            </Typography>
            <Divider sx={ my: 2 } />
...
export default C514_내부거래 제거통합 계산 관리자 업무 화면Screen;
