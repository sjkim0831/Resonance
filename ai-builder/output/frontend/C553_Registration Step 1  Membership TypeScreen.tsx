/**
 * Contract: #553 | Route: /join/en/step1
 * Process: MEMBER_REGISTRATION | Actor: PUBLIC_APPLICANT
 * Screen: Registration Step 1 - Membership Type
 * Generated: 2026-07-25T06:45:29.043285
 */
...
export const C553_Registration Step 1  Membership TypeScreen = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      // GET: session
      const r = await fetch('/join/api/session');
      const d = await r.json(); setData(d);
      // POST: step1
      const r = await fetch('/join/api/step1',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)});
      const d = await r.json();
      // POST: reset
      const r = await fetch('/join/api/reset',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)});
      const d = await r.json();
    } catch (err) {
  ...
  return (
    <Card>
      <CardHeader title="Registration Step 1 - Membership Type" subheader="Contract-based" />
      <CardContent>
        ...
        {!error && screenState !== 'LOADING' && (
          <Box>
            <Typography variant="body2" color="textSecondary">
              Process: MEMBER_REGISTRATION | Actor: PUBLIC_APPLICANT | Contract: #553
            </Typography>
            <Divider sx={ my: 2 } />
...
export default C553_Registration Step 1  Membership TypeScreen;
