/**
 * Contract: #12 | Route: /admin/emission/survey-report
 * Process: EMISSION_PROJECT | Actor: COMPANY_MANAGER
 * Screen: 보고서 생성·제출·진위 확인 관리자 업무 화면
 * Generated: 2026-07-25T06:45:29.039389
 */
...
export const C12_보고서 생성제출진위 확인 관리자 업무 화면Screen = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      // GET: reports
      const r = await fetch('/home/api/emission-projects/{id}/reports');
      const d = await r.json(); setData(d);
      // POST: reports
      const r = await fetch('/home/api/emission-projects/{id}/reports',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)});
      const d = await r.json();
      // POST: finalize
      const r = await fetch('/home/api/emission-projects/{id}/reports/{reportId}/finalize',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)});
      const d = await r.json();
      // POST: issue
      const r = await fetch('/home/api/emission-projects/{id}/reports/{reportId}/issue',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)});
      const d = await r.json();
      // POST: download
      const r = await fetch('/home/api/emission-projects/{id}/reports/{reportId}/download',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)});
      const d = await r.json();
    } catch (err) {
  ...
  return (
    <Card>
      <CardHeader title="보고서 생성·제출·진위 확인 관리자 업무 화면" subheader="Contract-based" />
      <CardContent>
        ...
        {!error && screenState !== 'LOADING' && (
          <Box>
            <Typography variant="body2" color="textSecondary">
              Process: EMISSION_PROJECT | Actor: COMPANY_MANAGER | Contract: #12
            </Typography>
            <Divider sx={ my: 2 } />
...
export default C12_보고서 생성제출진위 확인 관리자 업무 화면Screen;
