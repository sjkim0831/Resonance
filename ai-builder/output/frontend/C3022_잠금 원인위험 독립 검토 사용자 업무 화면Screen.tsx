/**
 * Contract: #3022 | Route: /signin/findPassword/result
 * Process: ACCOUNT_LOCK_RECOVERY | Actor: MEMBER_ADMIN
 * Screen: 잠금 원인·위험 독립 검토 사용자 업무 화면
 * Generated: 2026-07-25T06:45:29.046237
 */
...
export const C3022_잠금 원인위험 독립 검토 사용자 업무 화면Screen = () => {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      // GET: LOAD_RECOVERY_REVIEW
      const r = await fetch('/admin/api/member-recovery/{requestId}');
      const d = await r.json(); setData(d);
      // POST: REVIEW_ACCOUNT_RECOVERY
      const r = await fetch('/admin/api/member-recovery/{requestId}/review',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)});
  ...
  return (
    <Card>
      <CardHeader title="잠금 원인·위험 독립 검토 사용자 업무 화면" subheader="Contract-based" />
      <CardContent>
        ...
        {!error && screenState !== 'LOADING' && (
          <Box>
            <Typography variant="body2" color="textSecondary">
              Process: ACCOUNT_LOCK_RECOVERY | Actor: MEMBER_ADMIN | Contract: #3022
            </Typography>
            <Divider sx={ my: 2 } />
...
export default C3022_잠금 원인위험 독립 검토 사용자 업무 화면Screen;
