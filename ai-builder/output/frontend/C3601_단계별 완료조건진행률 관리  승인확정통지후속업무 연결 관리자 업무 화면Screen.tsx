/**
 * Contract: #3601 | Route: /admin/generated/process-completion-policy/process-completion-policy-s4
 * Process: PROCESS_COMPLETION_POLICY | Actor: APPROVER
 * Screen: 단계별 완료조건·진행률 관리 - 승인·확정·통지·후속업무 연결 관리자 업무 화면
 * Generated: 2026-07-25T06:45:29.059968
 */
...
// PROCESS_COMPLETION_POLICY_COMPLETE: POST /home/api/process-executions/{executionId}/commands
// LOAD_DRAFT: GET /home/api/process-executions/draft
// SAVE_DRAFT: PUT /home/api/process-executions/draft

export const C3601_단계별 완료조건진행률 관리  승인확정통지후속업무 연결 관리자 업무 화면Screen = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const d = await r.json(); setData(d);
      // POST: PROCESS_COMPLETION_POLICY_COMPLETE
  ...
  return (
    <Card>
      <CardHeader title="단계별 완료조건·진행률 관리 - 승인·확정·통지·후속업무 연결 관리자 업무 화면" subheader="Contract-based" />
      <CardContent>
        ...
        {!error && screenState !== 'LOADING' && (
          <Box>
            <Typography variant="body2" color="textSecondary">
              Process: PROCESS_COMPLETION_POLICY | Actor: APPROVER | Contract: #3601
            </Typography>
            <Divider sx={ my: 2 } />
...
export default C3601_단계별 완료조건진행률 관리  승인확정통지후속업무 연결 관리자 업무 화면Screen;
