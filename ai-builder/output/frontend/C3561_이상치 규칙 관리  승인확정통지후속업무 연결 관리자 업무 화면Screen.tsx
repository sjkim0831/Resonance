/**
 * Contract: #3561 | Route: /admin/generated/outlier-rule-management/outlier-rule-management-s4
 * Process: OUTLIER_RULE_MANAGEMENT | Actor: APPROVER
 * Screen: 이상치 규칙 관리 - 승인·확정·통지·후속업무 연결 관리자 업무 화면
 * Generated: 2026-07-25T06:45:29.059011
 */
...
// OUTLIER_RULE_MANAGEMENT_COMPLETE: POST /home/api/process-executions/{executionId}/commands
// LOAD_DRAFT: GET /home/api/process-executions/draft
// SAVE_DRAFT: PUT /home/api/process-executions/draft

export const C3561_이상치 규칙 관리  승인확정통지후속업무 연결 관리자 업무 화면Screen = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const d = await r.json(); setData(d);
      // POST: OUTLIER_RULE_MANAGEMENT_COMPLETE
  ...
  return (
    <Card>
      <CardHeader title="이상치 규칙 관리 - 승인·확정·통지·후속업무 연결 관리자 업무 화면" subheader="Contract-based" />
      <CardContent>
        ...
        {!error && screenState !== 'LOADING' && (
          <Box>
            <Typography variant="body2" color="textSecondary">
              Process: OUTLIER_RULE_MANAGEMENT | Actor: APPROVER | Contract: #3561
            </Typography>
            <Divider sx={ my: 2 } />
...
export default C3561_이상치 규칙 관리  승인확정통지후속업무 연결 관리자 업무 화면Screen;
