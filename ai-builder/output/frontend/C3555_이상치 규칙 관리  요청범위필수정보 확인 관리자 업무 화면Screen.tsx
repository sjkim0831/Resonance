/**
 * Contract: #3555 | Route: /admin/generated/outlier-rule-management/outlier-rule-management-s1
 * Process: OUTLIER_RULE_MANAGEMENT | Actor: DATA_ANALYST
 * Screen: 이상치 규칙 관리 - 요청·범위·필수정보 확인 관리자 업무 화면
 * Generated: 2026-07-25T06:45:29.058876
 */
...
// OUTLIER_RULE_MANAGEMENT_REQUEST: POST /home/api/process-executions/{executionId}/commands
// LOAD_DRAFT: GET /home/api/process-executions/draft
// SAVE_DRAFT: PUT /home/api/process-executions/draft

export const C3555_이상치 규칙 관리  요청범위필수정보 확인 관리자 업무 화면Screen = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const d = await r.json(); setData(d);
      // POST: OUTLIER_RULE_MANAGEMENT_REQUEST
  ...
  return (
    <Card>
      <CardHeader title="이상치 규칙 관리 - 요청·범위·필수정보 확인 관리자 업무 화면" subheader="Contract-based" />
      <CardContent>
        ...
        {!error && screenState !== 'LOADING' && (
          <Box>
            <Typography variant="body2" color="textSecondary">
              Process: OUTLIER_RULE_MANAGEMENT | Actor: DATA_ANALYST | Contract: #3555
            </Typography>
            <Divider sx={ my: 2 } />
...
export default C3555_이상치 규칙 관리  요청범위필수정보 확인 관리자 업무 화면Screen;
