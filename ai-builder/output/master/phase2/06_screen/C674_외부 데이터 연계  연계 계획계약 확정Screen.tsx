/**
 * Screen: 외부 데이터 연계 - 연계 계획·계약 확정
 * Contract: #674 | Route: /admin/system/process-workspace
 * Generated: 2026-07-25T09:34:04.558619
 */
...
// POST /home/api/process-executions/start
// POST /home/api/process-executions/{executionId}/commands

export const C674_외부 데이터 연계  연계 계획계약 확정Screen = () => {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [formData, setFormData] = useState({});
  const [screenState, setScreenState] = useState("LOADING, READY, EMPTY, VALIDATION_ERROR, FORBIDDEN");
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Typography variant="h5">외부 데이터 연계 - 연계 계획·계약 확정</Typography>
...
export default C674_외부 데이터 연계  연계 계획계약 확정Screen;