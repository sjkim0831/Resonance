/**
 * Screen: 인증서·PDF 발급 사용자 업무 화면
 * Contract: #2791 | Route: /emission/report_submit
 * Generated: 2026-07-25T09:34:04.560314
 */
...
export const C2791_인증서PDF 발급 사용자 업무 화면Screen = () => {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [formData, setFormData] = useState({});
  const [screenState, setScreenState] = useState("FINALIZED, LOADING, EMPTY, READY, SAVING");
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Typography variant="h5">인증서·PDF 발급 사용자 업무 화면</Typography>
...
export default C2791_인증서PDF 발급 사용자 업무 화면Screen;