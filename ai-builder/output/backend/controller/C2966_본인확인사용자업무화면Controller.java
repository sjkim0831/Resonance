...
public class C2966_본인확인사용자업무화면Controller {

    private final C2966_본인확인사용자업무화면Service service;
    ...
    public ResponseEntity<?> process_executions_screen_contract() {
        log.info("Contract #{}: {}", 2966, "SCREEN_CONTRACT");
        // Entry: 다음 프로세스 시작 조건을 충족한다: 비로그인 사용자가 홈 또는 로그인 화면에서 회원가입을 선택한다. 현재 상태는 CONSENTS_ACCEPTE
    ...
    public ResponseEntity<?> process_executions() {
        log.info("Contract #{}: {}", 2966, "LOAD_EXECUTION");
        // Entry: 다음 프로세스 시작 조건을 충족한다: 비로그인 사용자가 홈 또는 로그인 화면에서 회원가입을 선택한다. 현재 상태는 CONSENTS_ACCEPTE
        return ResponseEntity.ok().build();
    }
    ...
    public ResponseEntity<?> process_executions_executionId_commands(@PathVariable Long executionId) {
        log.info("Contract #{}: {}", 2966, "VERIFY_IDENTITY");
        // Entry: 다음 프로세스 시작 조건을 충족한다: 비로그인 사용자가 홈 또는 로그인 화면에서 회원가입을 선택한다. 현재 상태는 CONSENTS_ACCEPTE
        return ResponseEntity.ok().build();
    }
    ...
    public ResponseEntity<?> process_executions_draft() {
        log.info("Contract #{}: {}", 2966, "LOAD_DRAFT");
        // Entry: 다음 프로세스 시작 조건을 충족한다: 비로그인 사용자가 홈 또는 로그인 화면에서 회원가입을 선택한다. 현재 상태는 CONSENTS_ACCEPTE
        return ResponseEntity.ok().build();
    }
    ...
    public ResponseEntity<?> process_executions_draft() {
        log.info("Contract #{}: {}", 2966, "SAVE_DRAFT");
        // Entry: 다음 프로세스 시작 조건을 충족한다: 비로그인 사용자가 홈 또는 로그인 화면에서 회원가입을 선택한다. 현재 상태는 CONSENTS_ACCEPTE
        return ResponseEntity.ok().build();
    }
}
