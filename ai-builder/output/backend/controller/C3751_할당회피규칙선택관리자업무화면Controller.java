...
public class C3751_할당회피규칙선택관리자업무화면Controller {

    private final C3751_할당회피규칙선택관리자업무화면Service service;
    ...
    public ResponseEntity<?> process_executions_screen_contract() {
        log.info("Contract #{}: {}", 3751, "SCREEN_CONTRACT");
        // Entry: 다음 프로세스 시작 조건을 충족한다: 공정 산출물, 질량·경제가치·물리관계 자료가 준비되어 있다. 현재 상태는 STEP_1_COMPLETED이며
    ...
    public ResponseEntity<?> process_executions() {
        log.info("Contract #{}: {}", 3751, "LOAD_EXECUTION");
        // Entry: 다음 프로세스 시작 조건을 충족한다: 공정 산출물, 질량·경제가치·물리관계 자료가 준비되어 있다. 현재 상태는 STEP_1_COMPLETED이며
        return ResponseEntity.ok().build();
    }
    ...
    public ResponseEntity<?> process_executions_executionId_commands(@PathVariable Long executionId) {
        log.info("Contract #{}: {}", 3751, "LCA_ALLOCATION_SENSITIVITY_EXECUTE_2");
        // Entry: 다음 프로세스 시작 조건을 충족한다: 공정 산출물, 질량·경제가치·물리관계 자료가 준비되어 있다. 현재 상태는 STEP_1_COMPLETED이며
        return ResponseEntity.ok().build();
    }
    ...
    public ResponseEntity<?> process_executions_draft() {
        log.info("Contract #{}: {}", 3751, "LOAD_DRAFT");
        // Entry: 다음 프로세스 시작 조건을 충족한다: 공정 산출물, 질량·경제가치·물리관계 자료가 준비되어 있다. 현재 상태는 STEP_1_COMPLETED이며
        return ResponseEntity.ok().build();
    }
    ...
    public ResponseEntity<?> process_executions_draft() {
        log.info("Contract #{}: {}", 3751, "SAVE_DRAFT");
        // Entry: 다음 프로세스 시작 조건을 충족한다: 공정 산출물, 질량·경제가치·물리관계 자료가 준비되어 있다. 현재 상태는 STEP_1_COMPLETED이며
        return ResponseEntity.ok().build();
    }
}
