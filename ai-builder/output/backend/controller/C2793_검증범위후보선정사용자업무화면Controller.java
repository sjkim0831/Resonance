...
public class C2793_검증범위후보선정사용자업무화면Controller {

    private final C2793_검증범위후보선정사용자업무화면Service service;
    ...
    public ResponseEntity<?> process_executions_screen_contract() {
        log.info("Contract #{}: {}", 2793, "SCREEN_CONTRACT");
    ...
    public ResponseEntity<?> process_executions() {
        log.info("Contract #{}: {}", 2793, "LOAD_EXECUTION");
        // Entry: 다음 프로세스 시작 조건을 충족한다: 검증 범위와 후보 기관, 이해상충 정보가 준비되어 있다. 현재 상태는 READY이며 서버가 테넌트·프로젝트
        return ResponseEntity.ok().build();
    }
    ...
    public ResponseEntity<?> process_executions_executionId_commands(@PathVariable Long executionId) {
        log.info("Contract #{}: {}", 2793, "EXTERNAL_VERIFICATION_ENGAGEMENT_EXECUTE_1");
        // Entry: 다음 프로세스 시작 조건을 충족한다: 검증 범위와 후보 기관, 이해상충 정보가 준비되어 있다. 현재 상태는 READY이며 서버가 테넌트·프로젝트
        return ResponseEntity.ok().build();
    }
    ...
    public ResponseEntity<?> process_executions_draft() {
        log.info("Contract #{}: {}", 2793, "LOAD_DRAFT");
        // Entry: 다음 프로세스 시작 조건을 충족한다: 검증 범위와 후보 기관, 이해상충 정보가 준비되어 있다. 현재 상태는 READY이며 서버가 테넌트·프로젝트
        return ResponseEntity.ok().build();
    }
    ...
    public ResponseEntity<?> process_executions_draft() {
        log.info("Contract #{}: {}", 2793, "SAVE_DRAFT");
        // Entry: 다음 프로세스 시작 조건을 충족한다: 검증 범위와 후보 기관, 이해상충 정보가 준비되어 있다. 현재 상태는 READY이며 서버가 테넌트·프로젝트
        return ResponseEntity.ok().build();
    }
}
