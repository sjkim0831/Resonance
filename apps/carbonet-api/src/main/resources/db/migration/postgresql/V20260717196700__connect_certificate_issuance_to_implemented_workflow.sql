...
VALUES
('CERTIFICATE_ISSUANCE','CERTIFICATE_ISSUANCE_01_PLAN','CERT-PAGE-WORKFLOW','PAGE','보고서 확정·발급 업무 화면','/emission/report_submit','EmissionProjectReportPage',true,'VERIFIED','COMPANY_MANAGER','승인 보고서 확정부터 발급·다운로드까지 연결','source:EmissionProjectRegistryController'),
('CERTIFICATE_ISSUANCE','CERTIFICATE_ISSUANCE_02_WORK','CERT-API-ISSUE','API','인증서·PDF 멱등 발급 API','/home/api/emission-projects/{id}/reports/{reportId}/issue','EmissionProjectRegistryService.issueReportCertificate',true,'VERIFIED','APPROVER','동일 보고서 재호출 시 인증번호 불변, 감사 로그 저장','source:EmissionProjectRegistryService'),
('CERTIFICATE_ISSUANCE','CERTIFICATE_ISSUANCE_02_WORK','CERT-DATA-REGISTRY','DATA','발급 데이터셋·무결성·시각지문 저장','carbonet_report_verification_registry','ReportVerificationRegistryService',true,'VERIFIED','PLATFORM_OPERATOR','원본 데이터셋과 해시 및 페이지 시각 지문 보존','source:ReportVerificationRegistryService'),
('CERTIFICATE_ISSUANCE','CERTIFICATE_ISSUANCE_03_VERIFY','CERT-PAGE-VERIFY','PAGE','공개·관리자 진위 확인 화면','/home/certificate-verify','HomeCertificateVerifyPage',true,'VERIFIED','VERIFIER','로그인 없이 공개 검증, 관리자 상세 검증 가능','runtime:/home/certificate-verify=200'),
('CERTIFICATE_ISSUANCE','CERTIFICATE_ISSUANCE_03_VERIFY','CERT-API-VERIFY','API','데이터셋·OCR·시각지문 검증 API','/api/home/certificate-verify/verify-ocr','ReportVerificationRegistryService.verifyOcr',true,'VERIFIED','VERIFIER','정상·미등록·변조 문서를 구분하고 상세 차이를 반환','ops/scripts/verify-certificate-workflow.sh'),
('CERTIFICATE_ISSUANCE','CERTIFICATE_ISSUANCE_04_APPROVE','CERT-AUDIT-LIFECYCLE','AUDIT','발급·다운로드·폐기·재발급 감사 이력','emission_report_certificate_audit','EmissionProjectRegistryService',true,'VERIFIED','APPROVER','상태 전이마다 액터·사유·시각 기록','source:EmissionProjectRegistryService')
