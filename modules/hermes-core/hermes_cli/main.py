...
def _run_local_post_update_hooks(gateway_mode: bool = False) -> None:
    """Run site-local post-update hooks after upstream update work.

    Hooks are intentionally non-fatal: local operational glue should never make
    a successful upstream update look failed. This keeps Carbonet-specific
    commands, config tweaks, and service refreshes easy to reapply after future
    updates without forking Hermes.
    """
    hook_candidates = [
        PROJECT_ROOT / "scripts" / "local-post-update.sh",
        PROJECT_ROOT / "scripts" / "local-post-update.d",
        get_hermes_home() / "post-update.sh",
        get_hermes_home() / "post-update.d",
    ]
    hooks = []
    for candidate in hook_candidates:
        try:
            if candidate.is_file():
                ...
                hooks.extend(sorted(path for path in candidate.iterdir() if path.is_file()))
        except OSError:
            continue
    if not hooks:
        return
    ...
    env = {
        **os.environ,
        ...
        "HERMES_UPDATE_GATEWAY_MODE": "1" if gateway_mode else "0",
    }
    for hook in hooks:
        if not os.access(hook, os.X_OK):
            print(f"  - {hook}: skipped (not executable)")
            continue
        try:
            result = subprocess.run(
                [str(hook)],
            ...
            if result.returncode == 0:
                detail = f" - {output.splitlines()[-1]}" if output else ""
                print(f"  ✓ {hook.name}{detail}")
            else:
                detail = f": {output.splitlines()[-1]}" if output else ""
                print(f"  ⚠ {hook.name} exited {result.returncode}{detail}")
        except Exception as exc:
            print(f"  ⚠ {hook.name} failed: {exc}")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        _run_local_post_update_hooks(gateway_mode=gateway_mode)
...
""",
    ...
    # carbonet command
    # =========================================================================
    from hermes_cli.carbonet_orchestrator import register_carbonet_parser

    register_carbonet_parser(subparsers)
