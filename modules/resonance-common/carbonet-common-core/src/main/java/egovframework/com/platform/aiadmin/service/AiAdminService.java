...
public class AiAdminService {
    ...
    public Map<String, Object> buildModelsPage(String status, String provider, boolean en) {
        ...
        String sql = "SELECT model_name AS name, model_version AS version, provider, status, " +
                     ...
                     "created_at AS createdAt FROM ai_model_registry";
    ...
    public Map<String, Object> buildRagPage(String status, String source, boolean en) {
        ...
        try {
            documents = jdbc.queryForList(
                "SELECT id, document_name AS name, source, chunk_count AS chunkCount, " +
                "duplicate_rate AS duplicateRate, status, created_at AS createdAt " +
                "FROM ai_rag_document WHERE 1=1" + statusFilter + sourceFilter + " ORDER BY created_at DESC LIMIT 100"
            );
        } catch (Exception e) { log.warn("No documents found: {}", e.getMessage()); }

        try {
            chunks = jdbc.queryForList(
                "SELECT id, document_id, document_name AS docName, chunk_index AS chunkIndex, " +
                "content_preview AS content, token_count AS tokens, status, quality_score AS quality, " +
                "created_at AS createdAt " +
                "FROM ai_rag_chunk WHERE 1=1" + statusFilter + " ORDER BY created_at DESC LIMIT 200"
            );
    ...
    public Map<String, Object> buildAgentsPage(String status, boolean en) {
        ...
        try {
            agents = jdbc.queryForList(
                "SELECT team_id AS id, team_name AS name, service_name AS service, primary_model AS model, " +
                "active_yn AS status, frst_regist_pnttm AS createdAt " +
