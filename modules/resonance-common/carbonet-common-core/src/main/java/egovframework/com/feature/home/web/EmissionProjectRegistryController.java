...
public class EmissionProjectRegistryController {
    ...
    @PostMapping({"/home/api/emission-projects/{id}/activity-requests/{requestId}/decision","/en/home/api/emission-projects/{id}/activity-requests/{requestId}/decision"})
    public ResponseEntity<?> decideActivityRequest(@PathVariable String id,@PathVariable long requestId,@RequestBody Map<String,Object> body,HttpServletRequest request){var c=currentUserContextService.resolve(request);try{return ResponseEntity.ok(service.decideActivityRequest(id,requestId,tenant(c),c.getUserId(),c.isWebmaster(),body));}catch(SecurityException e){return ResponseEntity.status(403).body(Map.of("message",e.getMessage()));}catch(IllegalStateException e){return ResponseEntity.status(409).body(Map.of("message",e.getMessage()));}catch(Exception e){return ResponseEntity.badRequest().body(Map.of("message",e.getMessage()));}}
    ...
    @PostMapping({"/home/api/emission-projects/{id}/regulatory-submissions/{submissionId}/transition","/en/home/api/emission-projects/{id}/regulatory-submissions/{submissionId}/transition"})
    public ResponseEntity<?> transitionRegulatorySubmission(@PathVariable String id,@PathVariable long submissionId,@RequestBody Map<String,Object> body,HttpServletRequest request) {var context=currentUserContextService.resolve(request);if(!context.isAuthenticated())return ResponseEntity.status(401).body(Map.of("message","로그인이 필요합니다."));try{return ResponseEntity.ok(service.transitionRegulatorySubmission(id,submissionId,tenant(context),context.getUserId(),context.isWebmaster(),body));}catch(SecurityException e){return ResponseEntity.status(403).body(Map.of("message",e.getMessage()));}catch(IllegalStateException e){return ResponseEntity.status(409).body(Map.of("message",e.getMessage()));}catch(Exception e){return ResponseEntity.badRequest().body(Map.of("message",e.getMessage()));}}
    ...
    @GetMapping({"/home/api/emission-projects/{id}/organizational-boundary","/en/home/api/emission-projects/{id}/organizational-boundary"})
    public ResponseEntity<?> organizationalBoundary(@PathVariable String id,HttpServletRequest request){var c=currentUserContextService.resolve(request);if(!c.isAuthenticated())return ResponseEntity.status(401).body(Map.of("message","LOGIN_REQUIRED"));try{return ResponseEntity.ok(service.organizationalBoundary(id,tenant(c),c.getUserId(),c.isWebmaster()));}catch(SecurityException e){return ResponseEntity.status(403).body(Map.of("message",e.getMessage()));}catch(Exception e){return ResponseEntity.badRequest().body(Map.of("message",e.getMessage()));}}

    @PutMapping({"/home/api/emission-projects/{id}/organizational-boundary","/en/home/api/emission-projects/{id}/organizational-boundary"})
    public ResponseEntity<?> saveOrganizationalBoundary(@PathVariable String id,@RequestBody Map<String,Object> body,HttpServletRequest request){var c=currentUserContextService.resolve(request);if(!c.isAuthenticated())return ResponseEntity.status(401).body(Map.of("message","LOGIN_REQUIRED"));try{return ResponseEntity.ok(service.saveOrganizationalBoundary(id,tenant(c),c.getUserId(),c.isWebmaster(),body));}catch(SecurityException e){return ResponseEntity.status(403).body(Map.of("message",e.getMessage()));}catch(IllegalStateException e){return ResponseEntity.status(409).body(Map.of("message",e.getMessage()));}catch(Exception e){return ResponseEntity.badRequest().body(Map.of("message",e.getMessage()));}}
    ...
    @PostMapping({"/home/api/emission-projects/{id}/organizational-boundary/decision","/en/home/api/emission-projects/{id}/organizational-boundary/decision"})
    public ResponseEntity<?> decideBoundary(@PathVariable String id,@RequestBody Map<String,Object> body,HttpServletRequest request){var c=currentUserContextService.resolve(request);if(!c.isAuthenticated())return ResponseEntity.status(401).body(Map.of("message","LOGIN_REQUIRED"));try{return ResponseEntity.ok(service.decideOrganizationalBoundary(id,tenant(c),c.getUserId(),c.isWebmaster(),body));}catch(SecurityException e){return ResponseEntity.status(403).body(Map.of("message",e.getMessage()));}catch(IllegalStateException e){return ResponseEntity.status(409).body(Map.of("message",e.getMessage()));}catch(Exception e){return ResponseEntity.badRequest().body(Map.of("message",e.getMessage()));}}
    ...
    public ResponseEntity<?> companyManagerDelegations(@RequestParam(defaultValue="") String projectId,HttpServletRequest request) {var c=currentUserContextService.resolve(request);if(!c.isAuthenticated())return ResponseEntity.status(401).body(Map.of("message","AUTHENTICATION_REQUIRED"));try{return ResponseEntity.ok(service.companyManagerDelegationWorkspace(tenant(c),c.getUserId(),authorityAdmin(c),projectId));}catch(SecurityException e){return ResponseEntity.status(403).body(Map.of("message",e.getMessage()));}catch(Exception e){return ResponseEntity.badRequest().body(Map.of("message",e.getMessage()));}}
