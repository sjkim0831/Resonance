...
@Slf4j
public class AdminEmissionSurveyReportVerifyMenuBootstrapSupport {
    ...
    private static final String PAGE_CODE_ID = "AMENU1";
    private static final String MENU_CODE = "A0020513";
    private static final String MENU_NAME_KO = "리포트 진위확인";
    private static final String MENU_NAME_EN = "Report Authenticity";
    private static final String MENU_URL = "/admin/emission/survey-report-verify";
    private static final String MENU_ICON = "verified_user";
    ...
    public void ensureMenu() {
        try {
            ...
            if (adminCodeManageService.countPageManagementByCode(PAGE_CODE_ID, MENU_CODE) == 0) {
                adminCodeManageService.insertPageManagement(
                        PAGE_CODE_ID, MENU_CODE, MENU_NAME_KO, MENU_NAME_EN, MENU_URL, MENU_ICON, "Y", ACTOR_ID
                );
            } else {
                adminCodeManageService.updatePageManagement(
                        MENU_CODE, MENU_NAME_KO, MENU_NAME_EN, MENU_URL, MENU_ICON, "Y", ACTOR_ID
                );
            }
            log.info("Survey report authenticity menu provisioned. created={}, existing={}, skipped={}",
                    response.getCreatedCount(), response.getExistingCount(), response.getSkippedCount());
        } catch (Exception e) {
            log.error("Failed to provision survey report authenticity menu.", e);
        }
    }

    private CodexProvisionRequest buildRequest() {
        CodexProvisionRequest request = new CodexProvisionRequest();
        request.setRequestId("BOOTSTRAP-EMISSION-SURVEY-REPORT-VERIFY");
        ...
        request.setFeatures(Arrays.asList(
                featureRequest(MENU_CODE + "_VIEW", "진위확인 조회", "View Report Authenticity", "Open report authenticity verification"),
                featureRequest(MENU_CODE + "_VERIFY", "리포트 검증", "Verify Reports", "Upload and verify report PDF or images")
        ));
        request.setAuthors(Arrays.asList(
                authorRequest("ROLE_SYSTEM_MASTER", "시스템 마스터", "System Master", MENU_CODE + "_VIEW", MENU_CODE + "_VERIFY"),
                authorRequest("ROLE_SYSTEM_ADMIN", "시스템 관리자", "System Administrator", MENU_CODE + "_VIEW", MENU_CODE + "_VERIFY"),
                authorRequest("ROLE_ADMIN", "일반 관리자", "General Administrator", MENU_CODE + "_VIEW", MENU_CODE + "_VERIFY")
        ));
        return request;
    }
