...
public class ActorProcessGovernanceService {
    ...
    public Map<String,Object> pageDevelopmentMaster(String query,String processCode,String status){
        ...
        List<Map<String,Object>> items=jdbc.queryForList("select item_id as \"itemId\",sequence_no as \"sequenceNo\",priority_score as \"priorityScore\",screen_resource_id as \"screenResourceId\",route_key as \"routePath\",screen_name as \"screenName\",screen_type as \"screenType\",implementation_status as \"implementationStatus\",design_status as \"designStatus\",frontend_status as \"frontendStatus\",backend_status as \"backendStatus\",test_status as \"testStatus\",deployment_status as \"deploymentStatus\",menu_code as \"menuCode\",menu_name as \"menuName\",menu_status as \"menuStatus\",permission_code as \"permissionCode\",permission_name as \"permissionName\",permission_status as \"permissionStatus\",actor_codes as \"actorCodes\",process_codes as \"processCodes\",process_step_count as \"processStepCount\",capability_count as \"capabilityCount\",field_count as \"fieldCount\",quality_score as \"qualityScore\",customer_readiness as \"customerReadiness\",blocker_reason as \"blockerReason\",next_action as \"nextAction\" from framework_page_development_master where (?='' or lower(route_key||' '||screen_name||' '||actor_codes||' '||process_codes) like '%'||?||'%') and (?='' or position(? in process_codes)>0) and (?='' or design_status=? or frontend_status=? or backend_status=? or test_status=? or customer_readiness=?) order by sequence_no",keyword,keyword,process,process,state,state,state,state,state,state);
        Map<String,Object> summary=jdbc.queryForMap("select count(*) as \"total\",count(*) filter(where customer_readiness='CUSTOMER_READY') as \"customerReady\",count(*) filter(where design_status='REVIEW_REQUIRED') as \"designRequired\",count(*) filter(where frontend_status not in('VERIFIED','IMPLEMENTED')) as \"frontendRequired\",count(*) filter(where backend_status not in('VERIFIED','IMPLEMENTED')) as \"backendRequired\",count(*) filter(where test_status<>'VERIFIED') as \"testRequired\",count(*) filter(where menu_status='CONNECTED') as \"menuConnected\",count(*) filter(where permission_status='DEFINED') as \"permissionDefined\" from framework_page_development_master");
        ...
        return Map.of("success",true,"summary",summary,"processes",processes,"items",items);
    }
    ...
    public Map<String,Object> pageDevelopmentMasterDetail(long itemId){
        ...
        return Map.of("success",true,"item",item,"bindings",bindings,"capabilities",capabilities,"fields",fields,"tests",tests);
    }
