...
public final class Opef75ffabd1ea36c784a6f1a093794e0396a42972Controller {
    ...
    public Opef75ffabd1ea36c784a6f1a093794e0396a42972Controller(
    ...
    @org.springframework.web.bind.annotation.PostMapping(path="/home/api/process-executions/{executionId}/commands/reduction_project_registration/reduction_project_registration_s3/user", consumes="application/json")
    public org.springframework.http.ResponseEntity<?> execute(
            ...
            @org.springframework.web.bind.annotation.RequestBody Opef75ffabd1ea36c784a6f1a093794e0396a42972Request request,
        ...
        if(context==null || !context.isAuthenticated() || context.getUserId()==null || context.getUserId().isBlank())
            return org.springframework.http.ResponseEntity.status(401).body(new Opef75ffabd1ea36c784a6f1a093794e0396a42972ErrorResponse("AUTHENTICATION_REQUIRED","Authentication is required.",false));
        if(request==null || request.actorCode()==null || request.actorCode().isBlank() || request.actualReduction()==null || request.baselineEmission()==null || request.baselineYear()==null || request.baselineYear().isBlank() || request.capex()==null || request.decidedAt()==null || request.decidedAt().isBlank() || request.decisionCode()==null || request.decisionCode().isBlank() || request.expectedReduction()==null || request.idempotencyKey()==null || request.idempotencyKey().isBlank() || request.opex()==null || request.ownerActorCode()==null || request.ownerActorCode().isBlank() || request.projectId()==null || request.projectId().isBlank() || request.reductionMethod()==null || request.reductionMethod().isBlank() || request.rejectionReasonCode()==null || request.rejectionReasonCode().isBlank() || request.reviewComment()==null || request.reviewComment().isBlank() || request.targetReduction()==null || request.targetYear()==null || request.targetYear().isBlank() || request.tenantId()==null || request.tenantId().isBlank())
            return org.springframework.http.ResponseEntity.badRequest().body(new Opef75ffabd1ea36c784a6f1a093794e0396a42972ErrorResponse("INVALID_REQUEST","Request failed",false));
        if(!"VERIFIER".equals(request.actorCode()))
            return org.springframework.http.ResponseEntity.status(403).body(new Opef75ffabd1ea36c784a6f1a093794e0396a42972ErrorResponse("ACCESS_DENIED","Access denied",false));
        ...
        payload.put("routePath","/generated/reduction-project-registration/reduction-project-registration-s3");
        ...
        try {
            try { payload.put("requestJson",objectMapper.writeValueAsString(business)); }
                ...
                return org.springframework.http.ResponseEntity.badRequest().body(new Opef75ffabd1ea36c784a6f1a093794e0396a42972ErrorResponse("INVALID_REQUEST","Request serialization failed",false));
            }
            ...
            if(!Boolean.TRUE.equals(result.get("success"))
                ...
                return org.springframework.http.ResponseEntity.status(500).body(new Opef75ffabd1ea36c784a6f1a093794e0396a42972ErrorResponse("INTERNAL_ERROR","Response contract mismatch",false));
            ...
            if(Boolean.TRUE.equals(result.get("idempotent"))) {
                ...
                Opef75ffabd1ea36c784a6f1a093794e0396a42972RecoveryResponse response;
                try { response=objectMapper.convertValue(responsePayload,Opef75ffabd1ea36c784a6f1a093794e0396a42972RecoveryResponse.class); }
                catch(IllegalArgumentException mismatch) {
                    return org.springframework.http.ResponseEntity.status(500).body(new Opef75ffabd1ea36c784a6f1a093794e0396a42972ErrorResponse("INTERNAL_ERROR","Response contract mismatch",false));
                }
                return org.springframework.http.ResponseEntity.status(200).body(response);
            }
            Opef75ffabd1ea36c784a6f1a093794e0396a42972SuccessResponse response;
            try { response=objectMapper.convertValue(responsePayload,Opef75ffabd1ea36c784a6f1a093794e0396a42972SuccessResponse.class); }
            catch(IllegalArgumentException mismatch) {
                return org.springframework.http.ResponseEntity.status(500).body(new Opef75ffabd1ea36c784a6f1a093794e0396a42972ErrorResponse("INTERNAL_ERROR","Response contract mismatch",false));
            }
            return org.springframework.http.ResponseEntity.status(200).body(response);
        }
        catch(SecurityException denied) { return org.springframework.http.ResponseEntity.status(403).body(new Opef75ffabd1ea36c784a6f1a093794e0396a42972ErrorResponse("ACCESS_DENIED","Access denied",false)); }
        catch(IllegalArgumentException invalid) { return org.springframework.http.ResponseEntity.badRequest().body(new Opef75ffabd1ea36c784a6f1a093794e0396a42972ErrorResponse("INVALID_REQUEST","Request failed",false)); }
        catch(IllegalStateException conflict) { return org.springframework.http.ResponseEntity.status(409).body(new Opef75ffabd1ea36c784a6f1a093794e0396a42972ErrorResponse("CONFLICT","Request conflicts with the current state",false)); }
        catch(Exception unexpected) { return org.springframework.http.ResponseEntity.status(500).body(new Opef75ffabd1ea36c784a6f1a093794e0396a42972ErrorResponse("INTERNAL_ERROR","Internal processing failed",false)); }
    }
}
