...
@javax.annotation.processing.Generated(value="canonical-design", comments="designHash=887e0d10f5572c269009772d3b2a9c5f20aa159463e8ae6e17ff525c9d69ca17;endpointHash=5fdb03b18f46b18bb4d2e81314321e86c9388c071f4a5a440a96e8fe8dcf70d3")
public final class Op8f45050fb767b86fac65c7961961f8407334c0c9Controller {
    ...
    public Op8f45050fb767b86fac65c7961961f8407334c0c9Controller(
    ...
    @org.springframework.web.bind.annotation.PostMapping(path="/home/api/process-executions/{executionId}/commands/reduction_target_planning/reduction_target_planning_s3/user", consumes="application/json")
    public org.springframework.http.ResponseEntity<?> execute(
            ...
            @org.springframework.web.bind.annotation.RequestBody Op8f45050fb767b86fac65c7961961f8407334c0c9Request request,
        ...
        if(context==null || !context.isAuthenticated() || context.getUserId()==null || context.getUserId().isBlank())
            return org.springframework.http.ResponseEntity.status(401).body(new Op8f45050fb767b86fac65c7961961f8407334c0c9ErrorResponse("AUTHENTICATION_REQUIRED","Authentication is required.",false));
        if(request==null || request.actorCode()==null || request.actorCode().isBlank() || request.actualReduction()==null || request.baselineEmission()==null || request.baselineYear()==null || request.baselineYear().isBlank() || request.capex()==null || request.decidedAt()==null || request.decidedAt().isBlank() || request.decisionCode()==null || request.decisionCode().isBlank() || request.expectedReduction()==null || request.idempotencyKey()==null || request.idempotencyKey().isBlank() || request.opex()==null || request.ownerActorCode()==null || request.ownerActorCode().isBlank() || request.projectId()==null || request.projectId().isBlank() || request.reductionMethod()==null || request.reductionMethod().isBlank() || request.rejectionReasonCode()==null || request.rejectionReasonCode().isBlank() || request.reviewComment()==null || request.reviewComment().isBlank() || request.targetReduction()==null || request.targetYear()==null || request.targetYear().isBlank() || request.tenantId()==null || request.tenantId().isBlank())
            return org.springframework.http.ResponseEntity.badRequest().body(new Op8f45050fb767b86fac65c7961961f8407334c0c9ErrorResponse("INVALID_REQUEST","Request failed",false));
        if(!"VERIFIER".equals(request.actorCode()))
            return org.springframework.http.ResponseEntity.status(403).body(new Op8f45050fb767b86fac65c7961961f8407334c0c9ErrorResponse("ACCESS_DENIED","Access denied",false));
        ...
        payload.put("routePath","/generated/reduction-target-planning/reduction-target-planning-s3");
        ...
        try {
            try { payload.put("requestJson",objectMapper.writeValueAsString(business)); }
                ...
                return org.springframework.http.ResponseEntity.badRequest().body(new Op8f45050fb767b86fac65c7961961f8407334c0c9ErrorResponse("INVALID_REQUEST","Request serialization failed",false));
            }
            ...
            if(!Boolean.TRUE.equals(result.get("success"))
                ...
                return org.springframework.http.ResponseEntity.status(500).body(new Op8f45050fb767b86fac65c7961961f8407334c0c9ErrorResponse("INTERNAL_ERROR","Response contract mismatch",false));
            ...
            if(Boolean.TRUE.equals(result.get("idempotent"))) {
                ...
                Op8f45050fb767b86fac65c7961961f8407334c0c9RecoveryResponse response;
                try { response=objectMapper.convertValue(responsePayload,Op8f45050fb767b86fac65c7961961f8407334c0c9RecoveryResponse.class); }
                catch(IllegalArgumentException mismatch) {
                    return org.springframework.http.ResponseEntity.status(500).body(new Op8f45050fb767b86fac65c7961961f8407334c0c9ErrorResponse("INTERNAL_ERROR","Response contract mismatch",false));
                }
                return org.springframework.http.ResponseEntity.status(200).body(response);
            }
            Op8f45050fb767b86fac65c7961961f8407334c0c9SuccessResponse response;
            try { response=objectMapper.convertValue(responsePayload,Op8f45050fb767b86fac65c7961961f8407334c0c9SuccessResponse.class); }
            catch(IllegalArgumentException mismatch) {
                return org.springframework.http.ResponseEntity.status(500).body(new Op8f45050fb767b86fac65c7961961f8407334c0c9ErrorResponse("INTERNAL_ERROR","Response contract mismatch",false));
            }
            return org.springframework.http.ResponseEntity.status(200).body(response);
        }
        catch(SecurityException denied) { return org.springframework.http.ResponseEntity.status(403).body(new Op8f45050fb767b86fac65c7961961f8407334c0c9ErrorResponse("ACCESS_DENIED","Access denied",false)); }
        catch(IllegalArgumentException invalid) { return org.springframework.http.ResponseEntity.badRequest().body(new Op8f45050fb767b86fac65c7961961f8407334c0c9ErrorResponse("INVALID_REQUEST","Request failed",false)); }
        catch(IllegalStateException conflict) { return org.springframework.http.ResponseEntity.status(409).body(new Op8f45050fb767b86fac65c7961961f8407334c0c9ErrorResponse("CONFLICT","Request conflicts with the current state",false)); }
        catch(Exception unexpected) { return org.springframework.http.ResponseEntity.status(500).body(new Op8f45050fb767b86fac65c7961961f8407334c0c9ErrorResponse("INTERNAL_ERROR","Internal processing failed",false)); }
    }
}
