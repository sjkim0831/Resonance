...
await mkdir(outDir,{recursive:true}); await rm(definitionsDir,{recursive:true,force:true}); await mkdir(definitionsDir,{recursive:true});
...
for (const screen of normalized) {
  ...
  await writeFile(resolve(definitionsDir,`${screen.id}.ts`),`import type { GeneratedScreenDefinition } from "../generatedScreenTypes";\nexport const ${symbol} = ${json(screen)} as const satisfies GeneratedScreenDefinition;\n`);
}
