...
export function WorkExecutionPage() {
  ...
  const readyToComplete = checks.every(check => check.passed) && prerequisitesReady;
  ...
  return <main className="mx-auto w-full max-w-7xl px-4 py-8 lg:px-8">
    ...
    <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,2fr)_22rem]">
      <div className="space-y-6">
        ...
        {prerequisiteItems.length > 0 && <section className={`krds-component rounded-2xl border p-5 ${prerequisitesReady ? "border-emerald-200 bg-emerald-50" : "border-amber-300 bg-amber-50"}`}><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="gov-text-heading-md font-black text-[#052b57]">{en ? "Administrator prerequisites" : "관리자 선행정보"}</h2><p className="gov-text-body-sm mt-2 text-slate-700">{en ? "Required registries are checked before this step can be saved or completed." : "현재 단계를 저장하거나 완료하기 전에 필수 원장과 담당자 배정을 확인합니다."}</p></div><strong className={`rounded-full px-3 py-2 text-sm ${prerequisitesReady ? "bg-emerald-700 text-white" : "bg-amber-700 text-white"}`}>{prerequisitesReady ? (en ? "READY" : "진입 가능") : `${value(prerequisiteReadiness,"blockingMissingCount")} ${en ? "blocked" : "개 차단"}`}</strong></div><ul className="mt-4 grid gap-3 md:grid-cols-2">{prerequisiteItems.map((item,index) => <li className="rounded-xl border border-white bg-white p-4" key={`${value(item,"fieldCode")}-${index}`}><div className="flex items-start justify-between gap-3"><div><b>{value(item,"fieldName") || value(item,"fieldCode")}</b><p className="gov-text-label mt-1 text-slate-500">{value(item,"prerequisiteType")} · {item.blocking ? (en ? "required" : "필수") : (en ? "optional" : "선택")}</p></div><span className={`rounded-full px-2 py-1 text-xs font-black ${item.ready ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-900"}`}>{item.ready ? (en ? "Ready" : "준비") : (en ? "Missing" : "누락")}</span></div>{!item.ready && <><p className="gov-text-body-sm mt-3 text-slate-700">{value(item,"resolution")}</p><a className="mt-3 inline-flex min-h-10 items-center rounded-lg border border-[#246beb] px-3 text-sm font-black text-[#246beb]" href={value(item,"managementUrl")}>{en ? "Open management" : "관리 화면 열기"}</a></>}</li>)}</ul></section>}
      ...
      <aside className="space-y-5 xl:sticky xl:top-24 xl:self-start">
        ...
        <section className="krds-component rounded-2xl border bg-white p-5"><h2 className="gov-text-heading-sm font-black text-[#052b57]">{en ? "Work actions" : "업무 실행"}</h2><div className="mt-4 grid gap-3"><button className="krds-control rounded-lg border border-[#246beb] bg-white font-black text-[#246beb] disabled:opacity-50" disabled={busy || !actorCode || !prerequisitesReady} onClick={() => void saveDraft()}>{en ? "Save draft" : "임시저장"}</button>{!execution.found && <button className="krds-control rounded-lg bg-[#052b57] font-black text-white disabled:opacity-50" disabled={busy || !actorCode || !prerequisitesReady} onClick={() => void startExecution()}>{en ? "Start process" : "프로세스 시작"}</button>}<button className="krds-control rounded-lg bg-[#246beb] font-black text-white disabled:opacity-50" disabled={busy || !readyToComplete} onClick={() => void complete()}>{en ? "Validate and complete" : "검증 후 단계 완료"}</button></div><p className="gov-text-body-sm mt-4 text-slate-500">{prerequisitesReady ? (en ? "Completion is idempotent and server-authoritative." : "완료 명령은 서버 검증과 멱등성 규칙으로 보호됩니다.") : (en ? "Resolve administrator prerequisites before continuing." : "관리자 선행정보를 보완한 뒤 계속할 수 있습니다.")}</p></section>
