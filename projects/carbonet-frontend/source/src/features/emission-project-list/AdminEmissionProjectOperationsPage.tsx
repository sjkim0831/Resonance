...
export function AdminEmissionProjectOperationsPage() {
  ...
  const visibleItems = useMemo(
    () => (status ? data.items.filter((item) => item.status === status) : data.items),
    [data.items, status],
  );
  const visibleTotal = status
    ? data.summary.find((row) => row.status === status)?.count ?? visibleItems.length
    : data.total;
  const completed = data.items.filter((item) => item.progress >= 100).length;
  const delayed = data.items.filter(
    (item) => item.dueDate && new Date(item.dueDate) < new Date() && item.progress < 100,
  ).length;
    ...
    >
      <div className="space-y-5" data-testid="admin-emission-project-operations" data-active-menu-code={workspace.menuCode}>
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {[
            [en ? "Projects" : "전체 프로젝트", data.total],
            [en ? "Visible complete" : "현재 목록 완료", completed],
            [en ? "Delayed" : "마감 지연", delayed],
            [en ? "Active" : "진행 중", Math.max(data.items.length - completed, 0)],
          ].map(([label, value]) => (
            <article className="rounded-2xl border bg-white p-5" key={String(label)}>
              <span className="text-sm font-bold text-slate-500">{label}</span>
              <strong className="mt-2 block text-3xl font-black text-[#052b57]">{value}</strong>
            </article>
          ))}
        </section>
        ...
        {loading && (
          <p className="rounded-xl border bg-white p-8 text-center font-bold text-slate-600" role="status">
            {en ? "Loading projects..." : "프로젝트를 불러오는 중입니다."}
          </p>
        )}
        {error && (
          <p className="rounded-xl border border-red-200 bg-red-50 p-4 font-bold text-red-700" role="alert">
            {en ? "Failed to load project operations." : "프로젝트 운영 정보를 불러오지 못했습니다."} ({error})
          </p>
        )}
        {!loading && !error && (
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[980px] text-left text-sm">
                <thead className="bg-slate-100">
                  <tr>
                    {(en
                      ? ["Project", "Site / Scope", "Owner", "Current step", "Progress", "Due", "Status", "Action"]
                      : ["프로젝트", "사업장·Scope", "담당자", "현재 단계", "진행률", "마감일", "상태", "업무"]
                    ).map((head) => (
                  ...
                  {visibleItems.length ? (
                    visibleItems.map((item) => (
                      <tr className="border-t" key={item.id}>
                        ...
                        <td className="px-4 py-4">
                          {item.site}
                          <div className="text-xs text-slate-500">{item.scope} · {item.period}</div>
                        </td>
                        <td className="px-4 py-4">{item.owner}</td>
                        <td className="px-4 py-4">{item.step}</td>
                        <td className="px-4 py-4">
                          <b>{item.progress}%</b>
                          <div className="mt-2 h-2 w-24 overflow-hidden rounded bg-slate-100">
                            <div className="h-full bg-[#246beb]" style={{ width: `${Math.min(item.progress, 100)}%` }} />
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-4">
                          <a
                            ...
                            href={buildLocalizedPath(
                              `/emission/project/detail?id=${item.id}`,
                              `/en/emission/project/detail?id=${item.id}`,
                            )}
                          >
                            {en ? "Open" : "열기"}
                          </a>
                    ...
                    <tr>
                      <td className="px-4 py-12 text-center text-slate-500" colSpan={8}>
                        {en ? "No projects match the filters." : "조건에 맞는 프로젝트가 없습니다."}
                      </td>
                    </tr>
                  )}
                </tbody>
        ...
        <div className="flex items-center justify-between">
          <span className="text-sm text-slate-500">{en ? `${visibleTotal} projects` : `총 ${visibleTotal}개`}</span>
          <div className="flex gap-2">
            <button
              className="rounded-lg border bg-white px-4 py-2 font-bold disabled:opacity-40"
              disabled={page <= 1}
