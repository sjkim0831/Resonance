...
export function EmissionProjectListMigrationPage() {
  ...
  return <><HomeInlineStyles en={en} /><div className="min-h-screen bg-[#f6f8fb] text-[var(--kr-gov-text-primary)]">
    ...
    <main className="mx-auto max-w-7xl px-4 py-10 lg:px-8" id="project-list-main">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-bold text-[#246beb]">{en ? "Carbon Emission Management" : "탄소배출 관리"}</p><h1 className="mt-1 text-3xl font-black text-[#052b57]">{en ? "Emission Projects" : "배출량 프로젝트"}</h1><p className="mt-2 text-sm text-slate-600">{en ? "Find a project and continue its next required task." : "프로젝트를 찾아 현재 단계의 다음 업무를 진행합니다."}</p></div><div className="flex flex-wrap gap-2"><a className="inline-flex min-h-12 items-center justify-center gap-2 rounded-lg border border-[#246beb] bg-white px-5 font-black text-[#246beb] hover:bg-blue-50" href={buildLocalizedPath("/emission/project-portfolio", "/en/emission/project-portfolio")}><span className="material-symbols-outlined">dashboard</span>{en ? "Portfolio" : "포트폴리오"}</a><a className="inline-flex min-h-12 items-center justify-center gap-2 rounded-lg bg-[#246beb] px-5 font-black text-white hover:bg-[#164f86]" href={buildLocalizedPath("/emission/project/create", "/en/emission/project/create")}><span className="material-symbols-outlined">add</span>{en ? "New Project" : "새 프로젝트 등록"}</a></div></div>
