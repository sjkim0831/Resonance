...
type Detail = { success?: boolean; item?: Row; designGate?: Row; bindings?: Row[]; contracts?: Row[]; capabilities?: Row[]; fields?: Row[]; tests?: Row[]; assets?: Row[]; blueprints?: Row[]; registeredLayouts?: Row[]; registeredThemes?: Row[]; message?: string };
...
export function PageDesignStudioPage() {
  ...
  const selectedBlueprint = useMemo(()=>(detail.blueprints??[]).find(row=>text(row,"contractId")===selectedId),[detail.blueprints,selectedId]);
  useEffect(()=>{if(selected)setDraft({...selected,layout:text(selectedBlueprint,"layout"),theme:text(selectedBlueprint,"theme")});},[selected,selectedBlueprint]);
  ...
  const save=async()=>{
    ...
    try{
      ...
      const generationStatus=body.generationQueued===true?"코드 작업 등록":"구조화 설계 보완 대기";
      setMessage(`저장 완료 · ${generationStatus} · 작업 ${Number(body.jobCount??0)}개 · 엔드포인트 ${Number(body.endpointExpected??0)}개 · ${body.status??"저장됨"}`);await load();
      ...
      <section className="grid min-h-[680px] gap-4 xl:grid-cols-[260px_minmax(0,1fr)_390px]">
        ...
        <main className="space-y-4 overflow-hidden">
          ...
          <section className="rounded-2xl border bg-white"><nav className="flex overflow-x-auto border-b p-2">{[["contract","설계 계약"],["frontend","프론트"],["backend","백엔드·DB"],["tests","테스트"],["assets","공통 자산"],["preview","생성 미리보기"]].map(([id,label])=><button key={id} type="button" onClick={()=>setTab(id)} className={`whitespace-nowrap rounded-lg px-4 py-2 text-sm font-bold ${tab===id?"bg-[#052b57] text-white":"text-slate-600"}`}>{label}</button>)}</nav><div className="max-h-[610px] overflow-y-auto p-4">
            {tab==="contract"&&<ContractEditor draft={draft} setDraft={setDraft} layouts={detail.registeredLayouts??[]} themes={detail.registeredThemes??[]}/>}
            ...
            {tab==="preview"&&<SimpleTable rows={detail.blueprints??[]} columns={[["blueprintCode","블루프린트"],["screenType","화면 유형"],["templateCode","템플릿"],["layout","레이아웃"],["theme","테마"],["validationStatus","검증"]]}/>}
          </div></section>
        </main>
        <aside className="rounded-2xl border bg-white p-4"><h3 className="font-black text-[#052b57]">계약·검증·생성</h3><p className="mt-1 text-xs leading-5 text-slate-500">구조화 레이아웃·기능·화면 권한·API 설계는 저장 즉시 코드 작업으로 등록합니다. QA·시각·E2E 결과는 생성 후 게시·배포 검증에 사용합니다.</p><div className="mt-4 grid gap-2"><button type="button" disabled={busy||!selectedId} onClick={()=>void save()} className="min-h-11 rounded-lg bg-[#246beb] px-4 font-bold text-white disabled:opacity-40">설계 저장·코드 자동 반영</button><button type="button" disabled={busy||!processCode} onClick={()=>void execute("/design/validate",{processCode})} className="min-h-11 rounded-lg border border-blue-300 px-4 font-bold text-blue-700 disabled:opacity-40">프로세스 전체 검증</button><button type="button" disabled={busy||!processCode} onClick={()=>void execute("/development/direct",{processCode,force:false})} className="min-h-11 rounded-lg bg-emerald-600 px-4 font-bold text-white disabled:bg-slate-300">전체 프로세스 코드 재시도</button></div><p className="mt-3 rounded-lg bg-amber-50 p-3 text-xs font-bold leading-5 text-amber-800">게시·배포 전 QA: {text(gate,"issues")||"현재 QA 검증 항목을 통과했습니다."}</p><div className="mt-4 grid grid-cols-2 gap-2 text-xs">{[["테마","/admin/system/theme-management"],["섹션","/admin/system/section-management"],["컴포넌트","/admin/system/component-management"],["CSS","/admin/system/css-management"],["API","/admin/system/api-management"],["DB","/admin/system/db-table-management"],["테스트","/admin/system/verification-asset-management"],["메뉴","/admin/system/menu"]].map(([label,href])=><a key={href} href={href} className="rounded-lg border p-3 font-bold text-blue-700">{label} 관리</a>)}</div></aside>
      </section>
    </div>
  </AdminPageShell>;
}

function ContractEditor({draft,setDraft,layouts,themes}:{draft:Row;setDraft:(row:Row)=>void;layouts:Row[];themes:Row[]}){return <div className="grid gap-3 lg:grid-cols-2"><RegisteredDesignCode label="등록 레이아웃" name="layout" options={layouts} draft={draft} setDraft={setDraft}/><RegisteredDesignCode label="등록 테마" name="theme" options={themes} draft={draft} setDraft={setDraft}/><label className="lg:col-span-2 text-xs font-bold text-slate-700">화면 권한 코드(JSON 배열)<textarea value={text(draft,"permissionCodes")||"[]"} onChange={event=>setDraft({...draft,permissionCodes:event.target.value})} rows={3} placeholder='["PROJECT_READ", "PROJECT_WRITE"]' className="mt-1 w-full rounded-lg border p-3 font-mono text-xs leading-5"/><span className="mt-1 block font-normal text-slate-500">등록된 액터와 화면 route·audience에 결합되며, 저장 즉시 endpoint 권한과 생성 코드에 반영됩니다.</span></label>{contractTextKeys.map(([key,label])=><label key={key} className={`text-xs font-bold text-slate-700 ${["businessPurpose","entryCondition","exitCondition"].includes(key)?"lg:col-span-1":"lg:col-span-2"}`}>{label}<textarea value={text(draft,key)} onChange={event=>setDraft({...draft,[key]:event.target.value})} rows={["businessPurpose","entryCondition","exitCondition"].includes(key)?3:5} className="mt-1 w-full rounded-lg border p-3 font-mono text-xs leading-5"/></label>)}<label className="lg:col-span-2 text-xs font-bold">감사 증적 참조<input value={text(draft,"auditEvidenceRef")} onChange={event=>setDraft({...draft,auditEvidenceRef:event.target.value})} className="mt-1 h-11 w-full rounded-lg border px-3"/></label><div className="lg:col-span-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{verificationKeys.map(([key,label])=><label key={key} className="flex items-center gap-2 rounded-lg border p-3 text-xs font-bold"><input type="checkbox" checked={bool(draft,key)} onChange={event=>setDraft({...draft,[key]:event.target.checked})}/>{label} 검증 완료</label>)}</div></div>}
function RegisteredDesignCode({label,name,options,draft,setDraft}:{label:string;name:"layout"|"theme";options:Row[];draft:Row;setDraft:(row:Row)=>void}){const current=text(draft,name);const registered=options.map(row=>text(row,"code")).filter(Boolean);return <label className="text-xs font-bold text-slate-700">{label}<select value={current} onChange={event=>setDraft({...draft,[name]:event.target.value})} className="mt-1 h-11 w-full rounded-lg border px-3" required><option value="">등록 기본값 선택</option>{current&&!registered.includes(current)&&<option value={current} disabled>{current} (미등록)</option>}{registered.map(code=><option key={code} value={code}>{code}</option>)}</select></label>}
