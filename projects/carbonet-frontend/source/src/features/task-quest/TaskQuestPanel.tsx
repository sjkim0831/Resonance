...
function workflowPhaseLabel(code: string | undefined, en: boolean) {
  const labels: Record<string, [string, string]> = {
    REGISTRATION_AUTH: ["가입·인증", "Registration & auth"], COMPANY_ONBOARDING: ["기업·조직 온보딩", "Company onboarding"], ACCOUNT_OPERATION: ["계정 운영", "Account operation"],
    PROJECT_DATA_CALCULATION: ["프로젝트·자료·산정", "Project, data & calculation"], VERIFICATION_SUBMISSION: ["검증·제출", "Verification & submission"], MRV_TRACE_CLOSE: ["MRV·추적·종료", "MRV, trace & close"],
    LCA_CALCULATION_REVIEW: ["LCA 산정·검토", "LCA calculation & review"], TARGET_PROJECT: ["목표·과제", "Targets & projects"], ANALYSIS_PERFORMANCE: ["분석·성과", "Analysis & performance"],
    MONITORING_ANALYSIS: ["관제·분석", "Monitoring & analysis"], SUPPLY_DEMAND_TRADE: ["공급·수요·거래", "Supply, demand & trade"], PAYMENT_SETTLEMENT: ["결제·정산", "Payment & settlement"],
    REPORT: ["보고서", "Report"], CERTIFICATE_VERIFICATION: ["인증·진위", "Certificate & verification"], CONTENT_SUPPORT: ["콘텐츠·지원", "Content & support"], EDUCATION_OPERATION: ["교육 운영", "Education operation"],
    AUTH_WORKFLOW: ["권한·워크플로", "Authority & workflow"], EXTERNAL_INTEGRATION: ["외부 연계", "External integration"], PLATFORM_OPERATION: ["플랫폼 운영", "Platform operation"], COMMON_WORK: ["공통 업무", "Common work"], NEW_WORK: ["신규 업무", "New work"]
  };
  const label = labels[String(code || "NEW_WORK")];
  return label ? label[en ? 1 : 0] : String(code || "-");
}

type CatalogProcess = NonNullable<QuestResponse["processCatalog"]>[number];
type CatalogStep = NonNullable<QuestResponse["processCatalogSteps"]>[number];

function contractSummary(value: string | undefined, en: boolean) {
  if (!value) return en ? "No additional contract" : "추가 계약 없음";
  try {
    const parsed = JSON.parse(value) as Record<string, unknown>;
    const keys = Object.keys(parsed);
    return keys.length ? keys.join(", ") : (en ? "No additional contract" : "추가 계약 없음");
  } catch {
    return value;
  }
}

function ProcessGuideActionBar({process,steps,stepIndex,processes,allVisible,en,onStepChange,onProcessChange}:{process:CatalogProcess;steps:CatalogStep[];stepIndex:number;processes:CatalogProcess[];allVisible:boolean;en:boolean;onStepChange:(index:number)=>void;onProcessChange:(code:string)=>void}) {
  const safeIndex = Math.max(0,Math.min(stepIndex,Math.max(0,steps.length-1)));
  const step = steps[safeIndex];
  const route = step?.userPath || (allVisible ? step?.adminPath : "") || process.targetUrl || "";
  const targetRoute = route && en && !route.startsWith("/en/") ? `/en${route}` : route;
  const prerequisiteNames = String(process.prerequisiteCodes || "").split(",").map((code)=>code.trim()).filter(Boolean).map((code)=>processes.find((item)=>item.processCode===code)?.processName || code);
  const nextProcess = processes.find((item)=>item.processCode===process.nextProcessCode);
  const developmentHref = buildLocalizedPath(`/admin/system/actor-process?process=${encodeURIComponent(process.processCode)}`,`/en/admin/system/actor-process?process=${encodeURIComponent(process.processCode)}`);
  return <section aria-label={en?"Process action guide":"프로세스 실행 길잡이"} className="sticky bottom-0 z-20 mb-5 rounded-2xl border-2 border-[#052b57] bg-white p-4 shadow-[0_-8px_30px_rgba(15,23,42,0.12)] sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wide text-[#246beb]">{en?"Common process action guide":"공통 프로세스 하단 업무 길잡이"}</p><h3 className="mt-1 text-lg font-black text-[#052b57]">{process.processName} · {safeIndex+1}/{Math.max(steps.length,1)}</h3><p className="mt-1 text-sm text-slate-600">{step?.stepName || (en?"Process design pending":"프로세스 단계 설계 대기")}</p></div><div className="text-right"><span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-black text-blue-800">{Number(process.completionScore||0)}%</span><p className="mt-2 text-xs font-bold text-slate-500">{process.completedTasks||0}/{process.requiredTasks||0} Task</p></div></div><div className="mt-4 grid gap-2 text-xs sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-xl bg-slate-50 p-3"><strong className="block text-slate-500">{en?"Actor / authority":"담당 액터·권한"}</strong><span className="mt-1 block font-bold text-slate-800">{step?.actorCode || process.ownerActorCode || "-"}</span></div><div className="rounded-xl bg-slate-50 p-3"><strong className="block text-slate-500">{en?"Prerequisites":"선행 업무"}</strong><span className="mt-1 block font-bold text-slate-800">{prerequisiteNames.join(" · ") || (en?"None":"없음")}</span></div><div className="rounded-xl bg-slate-50 p-3"><strong className="block text-slate-500">{en?"Required input":"필요 입력자료"}</strong><span className="mt-1 block break-words font-bold text-slate-800">{contractSummary(step?.inputContract,en)}</span></div><div className="rounded-xl bg-slate-50 p-3"><strong className="block text-slate-500">{en?"Expected output":"예상 산출물"}</strong><span className="mt-1 block break-words font-bold text-slate-800">{contractSummary(step?.outputContract,en)}</span></div></div><div className="mt-3 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-950"><strong>{en?"Completion rule":"완료 조건"}: </strong>{step?.completionRule || process.nextAction || (en?"Implementation evidence and tests are required.":"구현 증적과 테스트 통과가 필요합니다.")}</div><div className="mt-4 flex flex-wrap items-center justify-between gap-2"><div className="flex gap-2"><button className="min-h-10 rounded-lg border border-slate-300 px-3 text-xs font-black text-slate-700 disabled:opacity-40" disabled={safeIndex<=0} onClick={()=>onStepChange(Math.max(0,safeIndex-1))} type="button">{en?"Previous step":"이전 단계"}</button><button className="min-h-10 rounded-lg border border-slate-300 px-3 text-xs font-black text-slate-700 disabled:opacity-40" disabled={safeIndex>=steps.length-1} onClick={()=>onStepChange(Math.min(steps.length-1,safeIndex+1))} type="button">{en?"Next step":"다음 단계"}</button></div><div className="flex flex-wrap justify-end gap-2">{targetRoute?<a className="flex min-h-10 items-center rounded-lg bg-[#246beb] px-4 text-xs font-black text-white" href={targetRoute}>{en?"Start / continue work":"업무 시작·계속 진행"}</a>:allVisible?<a className="flex min-h-10 items-center rounded-lg bg-amber-500 px-4 text-xs font-black text-white" href={developmentHref}>{en?"Open development status":"개발 현황 열기"}</a>:<span className="flex min-h-10 items-center rounded-lg bg-slate-200 px-4 text-xs font-black text-slate-600">{en?"Page development pending":"페이지 개발 대기"}</span>}{nextProcess?<button className="min-h-10 rounded-lg bg-[#052b57] px-4 text-xs font-black text-white" onClick={()=>onProcessChange(nextProcess.processCode)} type="button">{en?`Next process: ${nextProcess.processName}`:`다음 프로세스: ${nextProcess.processName}`}</button>:<span className="flex min-h-10 items-center rounded-lg bg-emerald-100 px-4 text-xs font-black text-emerald-800">{en?"Workflow complete":"업무 흐름 종료"}</span>}</div></div></section>;
}
...
export function TaskQuestPanel() {
  ...
  const availableWorkTypes = useMemo(() => {
    ...
    return [...definitions.values()]
      ...
      .map((item) => ({ code: String(item.workTypeCode).toUpperCase(), count: counts.get(String(item.workTypeCode).toUpperCase()) || 0, definedCount: Number(item.definedProcessCount || 0), label: (en ? item.workTypeNameEn : item.workTypeName) || workTypeLabel(item.workTypeCode, en), description: item.description || "" }));
  }, [data?.workTypes, en, workflowItems]);
  const definedProcessTotal = useMemo(() => availableWorkTypes.reduce((sum, item) => sum + item.definedCount, 0), [availableWorkTypes]);
      ...
      <div aria-labelledby="task-process-map-title" aria-modal="true" className="fixed inset-0 z-[1400] flex items-center justify-center bg-slate-950/60 p-3 backdrop-blur-[2px] sm:p-6" role="dialog">
        ...
        <section className="relative flex max-h-[88vh] w-full max-w-[78rem] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
          ...
          <div className="overflow-y-auto bg-slate-50 px-5 py-5 sm:px-7 sm:py-6">
            {data?.workCatalogAudit ? <div className={`mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border px-4 py-3 ${Number(data.workCatalogAudit.processesWithoutSequence||0)===0?"border-emerald-200 bg-emerald-50 text-emerald-900":"border-red-200 bg-red-50 text-red-900"}`}><div><strong className="block text-sm">{en?"Business sequence audit":"업무 순서 등록 점검"}</strong><span className="text-xs">{en?"Every process must have an executable order.":"모든 프로세스가 실행 순서 원장에 등록되어야 합니다."}</span></div><span className="rounded-full bg-white px-3 py-1 text-sm font-black">{data.workCatalogAudit.processesWithoutSequence||0} {en?"missing":"개 누락"}</span></div>:null}
            <section className="mb-5 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
              <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-xs font-black uppercase tracking-wide text-[#246beb]">{en ? "Step 1 · Select work type" : "1단계 · 업무 종류 선택"}</p><h3 className="mt-1 text-lg font-black text-[#052b57]">{en ? "Available work types" : "현재 선택 가능한 업무 종류"}</h3><p className="mt-1 text-sm text-slate-600">{en ? "Counts show registered processes, not project task steps." : "괄호 안 개수는 프로젝트 단계가 아닌 등록된 업무 프로세스 수입니다."}</p></div><label className="text-sm font-bold text-slate-700">{en ? "Work type" : "업무 종류"}<select className="ml-2 min-h-10 rounded-lg border border-slate-300 bg-white px-3" onChange={(event) => selectWorkType(event.target.value)} value={selectedWorkType}><option value="ALL">{en ? "All work" : "전체 업무"} ({definedProcessTotal})</option>{availableWorkTypes.map((item) => <option key={item.code} value={item.code}>{item.label} ({item.definedCount})</option>)}</select></label></div>
              <div className="mt-4 flex gap-2 overflow-x-auto pb-1"><button className={`shrink-0 rounded-xl border px-4 py-3 text-left ${selectedWorkType === "ALL" ? "border-[#246beb] bg-blue-50 text-blue-900" : "border-slate-200 text-slate-700"}`} onClick={() => selectWorkType("ALL")} type="button"><strong className="block text-sm">{en ? "All work" : "전체 업무"}</strong><span className="text-xs">{definedProcessTotal} {en ? "registered processes" : "등록 프로세스"} · {workflowItems.length} {en ? "active steps" : "진행 단계"}</span></button>{availableWorkTypes.map((item) => <button className={`shrink-0 rounded-xl border px-4 py-3 text-left ${selectedWorkType === item.code ? "border-[#246beb] bg-blue-50 text-blue-900" : "border-slate-200 text-slate-700"}`} key={item.code} onClick={() => selectWorkType(item.code)} title={item.description} type="button"><strong className="block text-sm">{item.label}</strong><span className="text-xs">{item.definedCount} {en ? "registered processes" : "등록 프로세스"} · {item.count} {en ? "active steps" : "진행 단계"}</span></button>)}</div>
            </section>
            {selectedDefinedProcesses.length ? <section className="mb-5 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wide text-[#246beb]">{en?"Execution order":"업무 실행 순서"}</p><h3 className="mt-1 text-lg font-black text-[#052b57]">{en?"Select one process and proceed step by step":"프로세스를 선택해 하나씩 구현·검증"}</h3></div><span className="text-xs font-bold text-slate-500">{selectedDefinedProcesses.length} {en?"processes":"개 프로세스"}</span></div><div className="mt-4 overflow-x-auto pb-2"><ol className="flex min-w-max items-stretch gap-0">{selectedDefinedProcesses.map((process,index)=><li className="flex items-center" key={`sequence-${process.processCode}`}><button className={`flex min-h-32 w-56 flex-col rounded-xl border-2 p-3 text-left ${selectedCatalogProcessCode===process.processCode?"border-[#246beb] bg-blue-50":"border-slate-200 bg-white"}`} onClick={()=>selectCatalogProcess(process.processCode)} type="button"><div className="flex items-center justify-between gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#052b57] text-xs font-black text-white">{index+1}</span><span className="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-black text-slate-600">{process.workflowPhase}</span></div><strong className="mt-3 text-sm text-[#052b57]">{process.processName}</strong><div className="mt-auto pt-3"><div className="h-1.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-[#246beb]" style={{width:`${Math.max(0,Math.min(100,Number(process.completionScore||0)))}%`}} /></div><span className="mt-1 block text-[11px] font-bold text-slate-500">{process.completedTasks||0}/{process.requiredTasks||0} Task · {Number(process.completionScore||0)}%</span></div></button>{index<selectedDefinedProcesses.length-1?<span aria-hidden="true" className="material-symbols-outlined mx-2 text-2xl text-slate-300">arrow_forward</span>:null}</li>)}</ol></div></section>:null}
            ...
            {selectedCatalogProcess ? <ProcessGuideActionBar allVisible={Boolean(data.allVisible)} en={en} onProcessChange={selectCatalogProcess} onStepChange={setSelectedCatalogStep} process={selectedCatalogProcess} processes={data.processCatalog || []} stepIndex={selectedCatalogStep} steps={selectedCatalogSteps} /> : null}
