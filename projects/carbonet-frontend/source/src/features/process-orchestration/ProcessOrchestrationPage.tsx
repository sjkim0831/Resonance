  ...
  return <AdminPageShell breadcrumbs={[
    ...
    <div className="space-y-5">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">{[
        ["담당 액터", text(process, "ownerActorCode") || "-"], ["업무 단계", steps.length], ["테스트 시나리오", cases.length], ["개발 작업", `${completedJobs}/${jobs.length}`], ["설계 정확도", `${text(assurance, "designAccuracyScore") || 0}%`], ["업무 SLA", `${text(process, "slaHours") || "-"}시간`],
      ...
      <section className="grid gap-4 xl:grid-cols-3">
        ...
        <article className="rounded-2xl border bg-white p-5"><h3 className="font-black text-[#052b57]">통제 기준</h3><dl className="mt-3 grid grid-cols-2 gap-3 text-sm"><dt className="text-slate-500">위험도</dt><dd className="font-bold">{text(process, "riskLevel") || "-"}</dd><dt className="text-slate-500">검토 주기</dt><dd className="font-bold">{text(process, "reviewCycleDays") || "-"}일</dd><dt className="text-slate-500">설계 상태</dt><dd className="font-bold">{text(assurance, "assuranceStatus") || "-"}</dd><dt className="text-slate-500">진척도</dt><dd className="font-bold">{text(progress, "progressPercent") || 0}%</dd></dl></article>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <article className="rounded-2xl border bg-white p-5"><h3 className="font-black text-[#052b57]">검증 시나리오</h3><ul className="mt-4 space-y-3">{cases.map(row => <StatusRow key={text(row, "caseCode")} title={text(row, "caseName")} detail={`${text(row, "caseType")} · ${text(row, "severity")}`} status={text(row, "caseStatus")} ok={text(row, "caseStatus") === "APPROVED"} />)}</ul></article>
