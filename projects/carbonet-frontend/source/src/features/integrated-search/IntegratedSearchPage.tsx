...
export function IntegratedSearchPage() {
  ...
  const [activeTone, setActiveTone] = useState<SearchCandidate["tone"] | null>(null);
  const [selectedPost, setSelectedPost] = useState<SearchCandidate | null>(null);
  const [pageIndex, setPageIndex] = useState(1);
  const [scopes, setScopes] = useState<Record<SearchCandidate["tone"], boolean>>({ menu: true, work: true, post: true });
  const initialPayload = useMemo<HomePayload>(() => ({ isLoggedIn: false, isEn: en, homeMenu: [] }), [en]);
  const state = useAsyncValue<HomePayload>(() => fetchHomePayload(), [en], { initialValue: initialPayload, onError: () => undefined });
  ...
  const activeItems = activeTone ? results.filter((item) => item.tone === activeTone) : [];
  const pageCount = Math.max(1, Math.ceil(activeItems.length / 10));
  const pageItems = activeItems.slice((pageIndex - 1) * 10, pageIndex * 10);

  function resultRow(item: SearchCandidate, tone: SearchCandidate["tone"]) {
    const row = <><span className="material-symbols-outlined mt-0.5 text-[#246beb]">{{ menu: "menu", work: "check_box", post: "description" }[tone]}</span><span className="min-w-0 flex-1"><strong className="block text-base group-hover:text-[#164f86] group-hover:underline">{item.label}</strong>{item.description ? <span className="mt-1 line-clamp-2 block text-sm leading-6 text-slate-600">{item.description}</span> : null}<span className="mt-2 block truncate text-xs text-slate-400">{item.href}</span></span><span className="material-symbols-outlined text-slate-400">chevron_right</span></>;
    return tone === "post"
      ? <button className="group flex w-full items-start gap-4 px-2 py-5 text-left hover:bg-blue-50/60" key={`${tone}-${item.label}-${item.href}`} onClick={() => setSelectedPost(item)} type="button">{row}</button>
      : <a className="group flex items-start gap-4 px-2 py-5 hover:bg-blue-50/60" href={item.href} key={`${tone}-${item.label}-${item.href}`}>{row}</a>;
  }
  ...
  return <><HomeInlineStyles en={en} /><div className="min-h-screen bg-white text-[var(--kr-gov-text-primary)]">
    ...
    <main className="mx-auto max-w-7xl px-4 py-10 lg:px-8 lg:py-14" id="integrated-search-main">
      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-[240px_minmax(0,1fr)]">
        ...
        <div>{selectedPost ? <article><button className="mb-5 inline-flex items-center gap-1 font-bold text-[#164f86] hover:underline" onClick={() => setSelectedPost(null)} type="button"><span className="material-symbols-outlined">arrow_back</span>{en ? "Back to post list" : "게시글 목록으로"}</button><div className="border-y-2 border-[#052b57]"><header className="bg-slate-50 px-6 py-7"><p className="text-sm font-bold text-[#246beb]">{en ? "Post" : "게시글"}</p><h2 className="mt-2 text-2xl font-black leading-snug">{selectedPost.label}</h2></header><div className="min-h-64 px-6 py-8 text-base leading-8 text-slate-700">{selectedPost.description || (en ? "No content summary is available." : "등록된 본문 요약이 없습니다.")}</div></div><div className="mt-6 flex justify-center"><a className="inline-flex min-h-11 items-center justify-center rounded-lg border border-[#246beb] px-6 font-bold text-[#164f86] hover:bg-blue-50" href={selectedPost.href}>{en ? "Open original board" : "원문 게시판 열기"}</a></div></article> : activeTone ? <section><button className="mb-5 inline-flex items-center gap-1 font-bold text-[#164f86] hover:underline" onClick={() => { setActiveTone(null); setPageIndex(1); }} type="button"><span className="material-symbols-outlined">arrow_back</span>{en ? "Back to all results" : "통합검색 결과로"}</button><div className="flex items-center gap-2 border-b-2 border-[#052b57] pb-4"><span className="material-symbols-outlined text-[#246beb]">{{ menu: "grid_view", work: "task_alt", post: "article" }[activeTone]}</span><h2 className="text-xl font-black">{copy[activeTone]} <span className="text-[#246beb]">{activeItems.length}</span>{en ? "" : "건"}</h2></div><div className="divide-y divide-slate-200 border-b border-slate-200">{pageItems.map((item) => resultRow(item, activeTone))}</div>{pageCount > 1 ? <div className="mt-6 flex items-center justify-center gap-2">{Array.from({ length: pageCount }, (_, index) => index + 1).map((page) => <button className={`h-10 w-10 rounded-lg font-bold ${page === pageIndex ? "bg-[#246beb] text-white" : "border border-slate-300 bg-white"}`} key={page} onClick={() => setPageIndex(page)} type="button">{page}</button>)}</div> : null}</section> : <><p className="border-b-2 border-[#052b57] pb-4 text-xl font-black">{en ? "Search results" : "전체 검색결과"} <strong className="text-[#246beb]">{results.length}</strong>{en ? "" : "건"}</p>{state.loading ? <p className="py-16 text-center text-slate-500">{en ? "Loading search index..." : "검색 색인을 불러오는 중입니다."}</p> : GROUPS.map((tone) => { const items = results.filter((item) => item.tone === tone); const visible = items.slice(0, 5); return <section className="py-7" key={tone}><div className="flex items-center gap-2"><span className="material-symbols-outlined text-[#246beb]">{{ menu: "grid_view", work: "task_alt", post: "article" }[tone]}</span><h2 className="text-xl font-black">{copy[tone]} <span className="text-[#246beb]">{items.length}</span>{en ? "" : "건"}</h2></div><div className="mt-3 divide-y divide-slate-200 border-y border-slate-200">{visible.length ? visible.map((item) => resultRow(item, tone)) : <p className="py-10 text-center text-slate-500">{en ? "No matching results." : "일치하는 결과가 없습니다."}</p>}</div>{items.length > 5 ? <div className="mt-4 text-center"><button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-[#246beb] bg-white px-6 font-bold text-[#164f86] hover:bg-blue-50" onClick={() => { setActiveTone(tone); setPageIndex(1); }} type="button">{en ? `View all ${copy[tone]}` : `${copy[tone]} 더 보기`}<span className="material-symbols-outlined text-lg">arrow_forward</span></button></div> : null}</section>; })}</>}</div>
      </div>
    </main>
  </div></>;
}
