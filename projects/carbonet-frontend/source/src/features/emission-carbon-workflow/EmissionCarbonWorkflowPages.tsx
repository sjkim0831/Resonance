...
function WorkflowPage({pageKey}:{pageKey:PageKey}){
  ...
  const actionPath=(id:string)=>pageKey==="request"?`/emission/data_input?tab=submission&projectId=${id}`:pageKey==="external"?`/emission/data_input?tab=external&projectId=${id}`:pageKey==="finalize"?`/emission/project-completion?projectId=${id}`:`/emission/report_submit?projectId=${id}`;
