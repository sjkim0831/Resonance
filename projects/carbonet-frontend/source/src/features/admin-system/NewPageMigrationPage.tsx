...
export function NewPageMigrationPage() {
  ...
  useEffect(() => {
    logGovernanceScope("PAGE", "new-page", {
      route: window.location.pathname,
      menuCode: stringOf(page || null, "menuCode"),
      featureCount,
      manifestComponentCount,
      grantedRoleCount,
      selectedTrackId
    });
    logGovernanceScope("COMPONENT", "new-page-starter-tracks", {
      component: "new-page-starter-tracks",
      selectedTrackId,
      readyCount,
      pendingCount
    });
  }, [featureCount, grantedRoleCount, manifestComponentCount, page, readyCount, pendingCount, selectedTrackId]);

  useEffect(() => {
    function syncFromNavigation() {
      const nextTrackId = new URLSearchParams(window.location.search).get("track") || starterTracks[0].id;
