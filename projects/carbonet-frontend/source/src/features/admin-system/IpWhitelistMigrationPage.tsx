...
export function IpWhitelistMigrationPage() {
  ...
  useEffect(() => {
    function syncFromNavigation() {
      ...
      setFilters((current) => ({
        searchIp: nextSearchParams.get("searchIp") || current.searchIp,
        accessScope: nextSearchParams.get("accessScope") || current.accessScope,
        status: nextSearchParams.get("status") || current.status
      }));
    }
