...
const platformRouteCount = ownershipTraces.filter((trace) => trace.routeScope === "PLATFORM").length;
  ...
  useEffect(() => {
    logGovernanceScope("PAGE", "module-management", {
      route: window.location.pathname,
      query,
      scope,
      standardModuleCount: filteredStandardModules.length,
      systemModuleCount: filteredSystemModules.length,
      platformRouteCount,
      runtimeRouteCount
    });
    logGovernanceScope("COMPONENT", "module-management-table", {
      component: "module-management-table",
      standardModuleCount: filteredStandardModules.length,
      systemModuleCount: filteredSystemModules.length
    });
  }, [filteredStandardModules.length, filteredSystemModules.length, platformRouteCount, query, runtimeRouteCount, scope]);

  useEffect(() => {
    function syncFromNavigation() {
      const nextSearchParams = new URLSearchParams(window.location.search);
      setQuery(nextSearchParams.get("query") || "");
      setScope(nextSearchParams.get("scope") || "ALL");
    }
