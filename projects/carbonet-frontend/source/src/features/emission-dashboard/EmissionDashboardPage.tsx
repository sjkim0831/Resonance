...
function DashboardStyles() {
  return <style>{`
    ...
    .dashboard-table{width:100%;border-collapse:collapse}.dashboard-table th{background:#f8fafc;color:#475569;font-size:12px;text-align:left;padding:12px 14px;border-bottom:1px solid #e2e8f0;white-space:nowrap}.dashboard-table td{padding:14px;border-bottom:1px solid #eef2f7;font-size:13px;color:#334155;vertical-align:middle}
...
export function EmissionDashboardPage() {
  ...
  const filtered = useMemo(() => projects.filter((p) => {
    ...
    if (scopeFilter !== "ALL" && text(p.scope, "ALL") !== scopeFilter) return false;
    return true;
  }), [projectFilter, projects, scopeFilter, siteFilter, yearFilter]);
  ...
  return <>
    ...
    <div className="min-h-screen bg-[#f4f7fa] text-[var(--kr-gov-text-primary)]">
      ...
      <main id="main-content" className="mx-auto max-w-[1440px] px-4 py-7 lg:px-8 lg:py-10">
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-black uppercase tracking-[.14em] text-[var(--kr-gov-blue)]">{labels.eyebrow}</p><h1 className="mt-2 text-3xl font-black tracking-tight text-slate-950 lg:text-4xl">{labels.title}</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600 lg:text-base">{labels.description}</p></div><div className="flex flex-wrap gap-2"><a className="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-4 py-3 text-sm font-black text-slate-700 hover:bg-slate-50" href={buildLocalizedPath("/emission/project_list", "/en/emission/project_list")}><span className="material-symbols-outlined text-[18px]">list_alt</span>{labels.table}</a><a className="inline-flex items-center gap-2 rounded-lg bg-[var(--kr-gov-blue)] px-4 py-3 text-sm font-black text-white hover:bg-[var(--kr-gov-blue-hover)]" href={buildLocalizedPath("/emission/project/create", "/en/emission/project/create")}><span className="material-symbols-outlined text-[18px]">add</span>{labels.create}</a></div></div>

        <section className="dashboard-card mt-7 grid gap-4 p-4 sm:grid-cols-2 lg:grid-cols-5" aria-label="filters">
          <label className="text-xs font-bold text-slate-600">{labels.company}<input className="mt-2 h-11 w-full rounded-lg border border-slate-300 bg-slate-100 px-3 text-sm text-slate-700" value={payload.isLoggedIn ? "Resonance 테스트 기업" : "—"} disabled /></label>
          <label className="text-xs font-bold text-slate-600">{labels.project}<select className="mt-2 h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm" value={projectFilter} onChange={(e) => setProjectFilter(e.target.value)}><option value="ALL">{labels.all}</option>{projects.map((p) => <option key={projectId(p)} value={projectId(p)}>{projectName(p)}</option>)}</select></label>
          <label className="text-xs font-bold text-slate-600">{labels.year}<select className="mt-2 h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm" value={yearFilter} onChange={(e) => setYearFilter(e.target.value)}><option value="ALL">{labels.all}</option>{years.map((year) => <option key={year}>{year}</option>)}</select></label>
          <label className="text-xs font-bold text-slate-600">{labels.site}<select className="mt-2 h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm" value={siteFilter} onChange={(e) => setSiteFilter(e.target.value)}><option value="ALL">{labels.all}</option>{sites.map((site) => <option key={site}>{site}</option>)}</select></label>
          <label className="text-xs font-bold text-slate-600">{labels.scope}<select className="mt-2 h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm" value={scopeFilter} onChange={(e) => setScopeFilter(e.target.value)}><option value="ALL">{labels.all}</option><option value="SCOPE_1">Scope 1</option><option value="SCOPE_2">Scope 2</option><option value="SCOPE_3">Scope 3</option></select></label>
        </section>
