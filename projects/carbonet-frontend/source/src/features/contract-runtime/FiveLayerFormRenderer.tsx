...
type Props = { contract: FiveLayerScreenContract; values: Record<string, unknown>; onChange: (fieldCode: string, value: unknown) => void; optionSources?: Record<string, RuntimeOption[]>; sectionCodes?: string[]; disabled?: boolean };
...
function ContractInput({ field, value, en, disabled, runtimeOptions, onChange }: { field: ContractField; value: unknown; en: boolean; disabled?: boolean; runtimeOptions?: RuntimeOption[]; onChange: (value: unknown) => void }) {
  ...
  return <label className="text-sm font-bold" htmlFor={common.id}>
    ...
    {field.type === "SELECT" ? <select {...common} value={String(value ?? "")} onChange={(event) => onChange(event.target.value)}>
      {field.optionSource ? <option value="">{en ? field.optionSource.emptyLabelEn : field.optionSource.emptyLabelKo}</option> : null}
      {(runtimeOptions || field.options || []).map((option) => <option key={option.value} value={option.value}>{"label" in option ? option.label : en ? option.labelEn : option.labelKo}</option>)}
...
export function FiveLayerFormRenderer({ contract, values, onChange, optionSources = {}, sectionCodes, disabled }: Props) {
  ...
  return <div className="space-y-5" data-contract-id={contract.screen.contractId} data-contract-version={contract.version}>
    {sections.map((section) => {
      ...
      return <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm" key={section.code}>
        ...
        <div className={`mt-5 grid gap-5 ${columns}`}>{fields.map((field) => <ContractInput disabled={disabled} en={en} field={field} key={field.code} onChange={(value) => onChange(field.code, value)} runtimeOptions={field.optionSource ? optionSources[field.optionSource.code] : undefined} value={values[field.code]} />)}</div>
      </section>;
    })}
  </div>;
}
