...
export function CommonCenteredSystemCanvas({ blueprints, mappings, onOpen, onRefresh, pageDesigns, processes, sections, steps }: Props) {
  ...
  const visibleIds = useMemo(() => {
    ...
    return new Set(model.nodes.filter(node => (kindFilter === "ALL" || node.kind === kindFilter) && (!needle || `${node.label} ${node.sub} ${node.route}`.toLocaleLowerCase("ko-KR").includes(needle))).map(node => node.id));
  }, [kindFilter, model.nodes, query]);
