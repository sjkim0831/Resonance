...
export function GlobalUserGnbShell({ children }: { children: ReactNode }) {
  ...
  return (
    <>
          ...
          <div className="gov-home-header relative flex items-center">
            ...
            <div className={`ml-auto flex shrink-0 items-center ${en ? "gap-2" : "gap-3"}`}>
              ...
              {payload.isLoggedIn ? (
                ...
                <>
                  <a className="gov-home-header-action hidden items-center rounded-[var(--kr-gov-radius)] bg-[var(--kr-gov-blue)] font-bold text-white xl:inline-flex" href={buildLocalizedPath("/signin/loginView", "/en/signin/loginView")}>{content.login}</a>
                  <a className="gov-home-header-action hidden items-center rounded-[var(--kr-gov-radius)] border border-[var(--kr-gov-blue)] bg-white font-bold text-[var(--kr-gov-blue)] xl:inline-flex" href={buildLocalizedPath("/join/step1", "/join/en/step1")}>{content.signup}</a>
                </>
              )}
      ...
      {payload.isLoggedIn ? <TestAccountSwitcher /> : null}
