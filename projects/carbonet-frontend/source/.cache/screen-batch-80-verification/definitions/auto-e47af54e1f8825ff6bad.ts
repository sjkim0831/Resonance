...
export const screen_auto_e47af54e1f8825ff6bad = {
  ...
  "blueprintCode": "BP_AUTO_E47AF54E1F8825FF6BAD2AFD",
  "processCode": "BACKGROUND_DB_VERSION_IMPACT",
  "stepCode": "BACKGROUND_DB_VERSION_IMPACT_S3",
  "actorCode": "VERIFIER",
  "audience": "ADMIN",
  ...
  "specification": {
    ...
    "businessPurpose": "배경 DB 버전·영향 관리 프로세스의 버전 변경 영향 재계산 단계에서 VERIFIER 액터가 BACKGROUND_DB_VERSION_IMPACT_EXECUTE_3 명령을 안전하게 수행하여 다음 완료 기준을 달성한다: 버전 변경 영향 재계산의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.",
    "actorResponsibilities": [
      "VERIFIER 액터가 권한·업무분리 정책에 따라 버전 변경 영향 재계산 관리자 업무 화면 업무를 수행한다."
    ],
    "entryConditions": [
      "다음 프로세스 시작 조건을 충족한다: 매핑된 배경 데이터셋과 버전 메타데이터가 존재한다. 현재 상태는 STEP_2_COMPLETED이며 서버가 테넌트·프로젝트·액터 권한을 확인한 경우에만 진입한다."
    ],
    "exitConditions": [
      "다음 완료 기준을 검증한다: 버전 변경 영향 재계산의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다. 결과·버전·감사 증적을 저장한 뒤 STEP_3_COMPLETED 상태로 원자적으로 전이한다."
    ],
    "states": [
      "STEP_2_COMPLETED",
      "LOADING",
    ...
    "actions": [
      {
        "code": "BACKGROUND_DB_VERSION_IMPACT_EXECUTE_3",
        "label": "버전 변경 영향 재계산 실행",
        "transactional": true,
    ...
    "completionRule": "다음 완료 기준을 검증한다: 버전 변경 영향 재계산의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다. 결과·버전·감사 증적을 저장한 뒤 STEP_3_COMPLETED 상태로 원자적으로 전이한다.",
    "extensions": {
      "contractId": 3723,
      "sharedRuntime": true
    }
  ...
  "traceability": {
    "contractId": 3723,
    "requirementIds": [
      "BACKGROUND_DB_VERSION_IMPACT:BACKGROUND_DB_VERSION_IMPACT_S3:ADMIN"
    ],
