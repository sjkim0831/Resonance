...
export const screen_auto_5f767aa501271000d943 = {
  ...
  "specification": {
    ...
    "businessPurpose": "시스템·DB 모니터링·자가복구 프로세스의 시스템·DB 모니터링·자가복구 - 업무 실행·중간 결과 저장 단계에서 PLATFORM_OPERATOR 액터가 SYSTEM_MONITORING_RECOVERY_EXECUTE 명령을 안전하게 수행하여 다음 완료 기준을 달성한다: 필수 입력, 액터 권한, 테넌트·프로젝트 격리, 증적, 멱등성과 상태 전이가 모두 검증되어야 한다.",
    ...
    "completionRule": "다음 완료 기준을 검증한다: 필수 입력, 액터 권한, 테넌트·프로젝트 격리, 증적, 멱등성과 상태 전이가 모두 검증되어야 한다.. 결과·버전·감사 증적을 저장한 뒤 STEP_2_COMPLETED 상태로 원자적으로 전이한다.",
    "extensions": {
      "contractId": 3669,
      "sharedRuntime": true
    }
