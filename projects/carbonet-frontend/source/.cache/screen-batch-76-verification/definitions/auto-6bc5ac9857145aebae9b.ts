...
export const screen_auto_6bc5ac9857145aebae9b = {
  "id": "auto-6bc5ac9857145aebae9b",
  "blueprintCode": "BP_AUTO_6BC5AC9857145AEBAE9BD0D6",
  ...
  "pageName": "PCR 적합성·유효기간 판정 관리자 업무 화면",
  ...
  "specification": {
    ...
    "businessPurpose": "PCR·EPD·제품탄소발자국 검증 프로세스의 PCR 적합성·유효기간 판정 단계에서 LCA_PRACTITIONER 액터가 PCR_EPD_VERIFICATION_EXECUTE_2 명령을 안전하게 수행하여 다음 완료 기준을 달성한다: PCR 적합성·유효기간 판정의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.",
    "actorResponsibilities": [
      "LCA_PRACTITIONER 액터가 권한·업무분리 정책에 따라 PCR 적합성·유효기간 판정 관리자 업무 화면 업무를 수행한다."
    ],
    "entryConditions": [
      "다음 프로세스 시작 조건을 충족한다: 제품 분류, 시장, 적용 기간과 후보 PCR이 확인된다.. 현재 상태는 STEP_1_COMPLETED이며 서버가 테넌트·프로젝트·액터 권한을 확인한 경우에만 진입한다."
    ],
    "exitConditions": [
      "다음 완료 기준을 검증한다: PCR 적합성·유효기간 판정의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.. 결과·버전·감사 증적을 저장한 뒤 STEP_2_COMPLETED 상태로 원자적으로 전이한다."
    ],
    "states": [
      "STEP_1_COMPLETED",
      "LOADING",
    ...
    "apiContracts": [
      ...
      },
      {
        ...
        "method": "POST",
        "purpose": "PCR 적합성·유효기간 판정 상태 명령 실행"
      },
      {
        "code": "LOAD_DRAFT",
    ...
    "completionRule": "다음 완료 기준을 검증한다: PCR 적합성·유효기간 판정의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.. 결과·버전·감사 증적을 저장한 뒤 STEP_2_COMPLETED 상태로 원자적으로 전이한다.",
    "extensions": {
      "contractId": 3783,
      "sharedRuntime": true
    }
