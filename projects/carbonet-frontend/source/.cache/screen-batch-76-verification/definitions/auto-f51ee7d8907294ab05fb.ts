...
export const screen_auto_f51ee7d8907294ab05fb = {
  ...
  "specification": {
    ...
    "businessPurpose": "메뉴·페이지·화면 연결 관리 프로세스의 메뉴·페이지·화면 연결 관리 - 업무 실행·중간 결과 저장 단계에서 PLATFORM_OPERATOR 액터가 MENU_SCREEN_GOVERNANCE_EXECUTE 명령을 안전하게 수행하여 다음 완료 기준을 달성한다: 필수 입력, 액터 권한, 테넌트·프로젝트 격리, 증적, 멱등성과 상태 전이가 모두 검증되어야 한다.",
