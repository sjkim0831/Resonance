...
export const screen_auto_c1f897005364c03c208d = {
  ...
  "blueprintCode": "BP_AUTO_C1F897005364C03C208D57FF",
  "processCode": "PROJECT_LIFECYCLE_CONTROL",
  "stepCode": "PROJECT_LIFECYCLE_CONTROL_S3",
  "actorCode": "AUDITOR",
  "audience": "ADMIN",
  ...
  "specification": {
    ...
    "businessPurpose": "프로젝트 종료·보관·재개·취소 프로세스의 권한분리 승인 단계에서 AUDITOR 액터가 PROJECT_LIFECYCLE_CONTROL_EXECUTE_3 명령을 안전하게 수행하여 다음 완료 기준을 달성한다: 권한분리 승인의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.",
    "actorResponsibilities": [
      "AUDITOR 액터가 권한·업무분리 정책에 따라 권한분리 승인 관리자 업무 화면 업무를 수행한다."
    ],
    "entryConditions": [
      "다음 프로세스 시작 조건을 충족한다: 전환 요청자와 사유, 미결 태스크, 보존 정책이 확인된다. 현재 상태는 STEP_2_COMPLETED이며 서버가 테넌트·프로젝트·액터 권한을 확인한 경우에만 진입한다."
    ],
    "exitConditions": [
      "다음 완료 기준을 검증한다: 권한분리 승인의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다. 결과·버전·감사 증적을 저장한 뒤 STEP_3_COMPLETED 상태로 원자적으로 전이한다."
    ],
    "states": [
      "STEP_2_COMPLETED",
      "LOADING",
    ...
    "actions": [
      {
        "code": "PROJECT_LIFECYCLE_CONTROL_EXECUTE_3",
        "label": "권한분리 승인 실행",
        "transactional": true,
    ...
    "completionRule": "다음 완료 기준을 검증한다: 권한분리 승인의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다. 결과·버전·감사 증적을 저장한 뒤 STEP_3_COMPLETED 상태로 원자적으로 전이한다.",
    "extensions": {
      "contractId": 3793,
      "sharedRuntime": true
    }
  ...
  "traceability": {
    "contractId": 3793,
    "requirementIds": [
      "PROJECT_LIFECYCLE_CONTROL:PROJECT_LIFECYCLE_CONTROL_S3:ADMIN"
    ],
