...
export const screen_auto_6bc5ac9857145aebae9b = {
  ...
  "specification": {
    ...
    "businessPurpose": "PCR·EPD·제품탄소발자국 검증 프로세스의 PCR 적합성·유효기간 판정 단계에서 LCA_PRACTITIONER 액터가 PCR_EPD_VERIFICATION_EXECUTE_2 명령을 안전하게 수행하고 PCR 적합성·유효기간 판정의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.를 완료한다.",
    "actorResponsibilities": [
      "LCA_PRACTITIONER 액터가 권한·업무분리 정책에 따라 PCR 적합성·유효기간 판정 관리자 업무 화면 업무를 수행한다."
    ],
    "entryConditions": [
      "제품 분류, 시장, 적용 기간과 후보 PCR이 확인된다.을 충족하고 현재 상태가 STEP_1_COMPLETED이며 서버가 테넌트·프로젝트·액터 권한을 확인한 경우 진입한다."
    ],
    "exitConditions": [
      "PCR 적합성·유효기간 판정의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_2_COMPLETED 상태로 원자적으로 전이한다."
    ],
    "states": [
      "STEP_1_COMPLETED",
      "LOADING",
    ...
    "completionRule": "PCR 적합성·유효기간 판정의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_2_COMPLETED 상태로 원자적으로 전이한다.",
    "extensions": {
      "contractId": 3783,
      "sharedRuntime": true
    }
