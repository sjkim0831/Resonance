...
export const screen_auto_bee31330417707ecc65b = {
  ...
  "blueprintCode": "BP_AUTO_BEE31330417707ECC65BDBF2",
  ...
  "specification": {
    ...
    "businessPurpose": "배경 DB 버전·영향 관리 프로세스의 데이터셋·버전·라이선스 수집 단계에서 LCA_PRACTITIONER 액터가 BACKGROUND_DB_VERSION_IMPACT_EXECUTE_1 명령을 안전하게 수행하고 데이터셋·버전·라이선스 수집의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.를 완료한다.",
    "actorResponsibilities": [
      "LCA_PRACTITIONER 액터가 권한·업무분리 정책에 따라 데이터셋·버전·라이선스 수집 사용자 업무 화면 업무를 수행한다."
    ],
    "entryConditions": [
      "매핑된 배경 데이터셋과 버전 메타데이터가 존재한다.을 충족하고 현재 상태가 READY이며 서버가 테넌트·프로젝트·액터 권한을 확인한 경우 진입한다."
    ],
    "exitConditions": [
      "데이터셋·버전·라이선스 수집의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_1_COMPLETED 상태로 원자적으로 전이한다."
    ],
    "states": [
      "READY",
      "LOADING",
    ...
    "completionRule": "데이터셋·버전·라이선스 수집의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_1_COMPLETED 상태로 원자적으로 전이한다.",
    "extensions": {
      "contractId": 3718,
      "sharedRuntime": true
    }
  ...
  "traceability": {
    "contractId": 3718,
    "requirementIds": [
      "BACKGROUND_DB_VERSION_IMPACT:BACKGROUND_DB_VERSION_IMPACT_S1:USER"
    ],
