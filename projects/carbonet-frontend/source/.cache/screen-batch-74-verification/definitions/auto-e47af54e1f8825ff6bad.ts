...
export const screen_auto_e47af54e1f8825ff6bad = {
  ...
  "specification": {
    ...
    "businessPurpose": "배경 DB 버전·영향 관리 프로세스의 버전 변경 영향 재계산 단계에서 VERIFIER 액터가 BACKGROUND_DB_VERSION_IMPACT_EXECUTE_3 명령을 안전하게 수행하고 버전 변경 영향 재계산의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.를 완료한다.",
    "actorResponsibilities": [
      "VERIFIER 액터가 권한·업무분리 정책에 따라 버전 변경 영향 재계산 관리자 업무 화면 업무를 수행한다."
    ],
    "entryConditions": [
      "매핑된 배경 데이터셋과 버전 메타데이터가 존재한다.을 충족하고 현재 상태가 STEP_2_COMPLETED이며 서버가 테넌트·프로젝트·액터 권한을 확인한 경우 진입한다."
    ],
    "exitConditions": [
      "버전 변경 영향 재계산의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_3_COMPLETED 상태로 원자적으로 전이한다."
    ],
    "states": [
      "STEP_2_COMPLETED",
      "LOADING",
    ...
    "completionRule": "버전 변경 영향 재계산의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_3_COMPLETED 상태로 원자적으로 전이한다.",
    "extensions": {
      "contractId": 3723,
      "sharedRuntime": true
    }
