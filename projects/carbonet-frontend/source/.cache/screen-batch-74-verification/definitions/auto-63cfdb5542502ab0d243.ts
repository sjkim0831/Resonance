...
export const screen_auto_63cfdb5542502ab0d243 = {
  ...
  "specification": {
    ...
    "businessPurpose": "CCUS 전과정 MRV 프로세스의 시설경계·기준선 확정 단계에서 CALCULATOR 액터가 CCUS_LIFECYCLE_MRV_EXECUTE_1 명령을 안전하게 수행하고 시설경계·기준선 확정의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.를 완료한다.",
    "actorResponsibilities": [
      "CALCULATOR 액터가 권한·업무분리 정책에 따라 시설경계·기준선 확정 사용자 업무 화면 업무를 수행한다."
    ],
    "entryConditions": [
      "시설 경계, 측정 지점, 기준선과 모니터링 계획이 승인되어 있다.을 충족하고 현재 상태가 READY이며 서버가 테넌트·프로젝트·액터 권한을 확인한 경우 진입한다."
    ],
    "exitConditions": [
      "시설경계·기준선 확정의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_1_COMPLETED 상태로 원자적으로 전이한다."
    ],
    "states": [
      "READY",
      "LOADING",
    ...
    "completionRule": "시설경계·기준선 확정의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_1_COMPLETED 상태로 원자적으로 전이한다.",
    "extensions": {
      "contractId": 3724,
      "sharedRuntime": true
    }
  ...
  "traceability": {
    "contractId": 3724,
    "requirementIds": [
      "CCUS_LIFECYCLE_MRV:CCUS_LIFECYCLE_MRV_S1:USER"
    ],
