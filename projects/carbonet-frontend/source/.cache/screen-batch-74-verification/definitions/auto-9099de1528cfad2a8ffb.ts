...
export const screen_auto_9099de1528cfad2a8ffb = {
  ...
  "specification": {
    ...
    "businessPurpose": "외부 검증기관 계약·독립성·의견서 프로세스의 검증 수행·발견사항 조치 단계에서 AUDITOR 액터가 EXTERNAL_VERIFICATION_ENGAGEMENT_EXECUTE_3 명령을 안전하게 수행하고 검증 수행·발견사항 조치의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.를 완료한다.",
    "actorResponsibilities": [
      "AUDITOR 액터가 권한·업무분리 정책에 따라 검증 수행·발견사항 조치 관리자 업무 화면 업무를 수행한다."
    ],
    "entryConditions": [
      "검증 범위와 후보 기관, 이해상충 정보가 준비되어 있다.을 충족하고 현재 상태가 STEP_2_COMPLETED이며 서버가 테넌트·프로젝트·액터 권한을 확인한 경우 진입한다."
    ],
    "exitConditions": [
      "검증 수행·발견사항 조치의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_3_COMPLETED 상태로 원자적으로 전이한다."
    ],
    "states": [
      "STEP_2_COMPLETED",
      "LOADING",
    ...
    "completionRule": "검증 수행·발견사항 조치의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_3_COMPLETED 상태로 원자적으로 전이한다.",
    "extensions": {
      "contractId": 3077,
      "sharedRuntime": true
    }
