...
export const screen_auto_be12142fe3bd97e117a1 = {
  ...
  "specification": {
    ...
    "businessPurpose": "할당·민감도 분석 프로세스의 할당 회피·규칙 선택 단계에서 LCA_PRACTITIONER 액터가 LCA_ALLOCATION_SENSITIVITY_EXECUTE_2 명령을 안전하게 수행하고 할당 회피·규칙 선택의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.를 완료한다.",
    "actorResponsibilities": [
      "LCA_PRACTITIONER 액터가 권한·업무분리 정책에 따라 할당 회피·규칙 선택 관리자 업무 화면 업무를 수행한다."
    ],
    "entryConditions": [
      "공정 산출물, 질량·경제가치·물리관계 자료가 준비되어 있다.을 충족하고 현재 상태가 STEP_1_COMPLETED이며 서버가 테넌트·프로젝트·액터 권한을 확인한 경우 진입한다."
    ],
    "exitConditions": [
      "할당 회피·규칙 선택의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_2_COMPLETED 상태로 원자적으로 전이한다."
    ],
    "states": [
      "STEP_1_COMPLETED",
      "LOADING",
    ...
    "completionRule": "할당 회피·규칙 선택의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_2_COMPLETED 상태로 원자적으로 전이한다.",
    "extensions": {
      "contractId": 3751,
      "sharedRuntime": true
    }
