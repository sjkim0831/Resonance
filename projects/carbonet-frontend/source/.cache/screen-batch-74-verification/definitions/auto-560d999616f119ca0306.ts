...
export const screen_auto_560d999616f119ca0306 = {
  ...
  "specification": {
    ...
    "businessPurpose": "인증서 신청·발급·검증 프로세스의 폐기·재발급·감사 확정 단계에서 APPROVER 액터가 CLOSE_CERTIFICATE_LIFECYCLE 명령을 안전하게 수행하고 폐기·재발급 상태, 이전 인증번호, 다운로드 및 감사 이력이 일치한다.를 완료한다.",
    "actorResponsibilities": [
      "APPROVER 액터가 권한·업무분리 정책에 따라 폐기·재발급·감사 확정 관리자 업무 화면 업무를 수행한다."
    ],
    "entryConditions": [
      "승인된 산출 결과 존재을 충족하고 현재 상태가 VERIFIED이며 서버가 테넌트·프로젝트·액터 권한을 확인한 경우 진입한다."
    ],
    "exitConditions": [
      "폐기·재발급 상태, 이전 인증번호, 다운로드 및 감사 이력이 일치한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 COMPLETED 상태로 원자적으로 전이한다."
    ],
    "states": [
      "VERIFIED",
      "LOADING",
    ...
    "completionRule": "폐기·재발급 상태, 이전 인증번호, 다운로드 및 감사 이력이 일치한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 COMPLETED 상태로 원자적으로 전이한다.",
    "extensions": {
      "contractId": 2826,
      "sharedRuntime": true
    }
