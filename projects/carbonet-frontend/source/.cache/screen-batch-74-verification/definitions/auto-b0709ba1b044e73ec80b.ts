...
export const screen_auto_b0709ba1b044e73ec80b = {
  ...
  "specification": {
    ...
    "businessPurpose": "출처·이동·인수인계 연속성 프로세스의 양측 인계확인·차이 조정 단계에서 VERIFIER 액터가 CHAIN_OF_CUSTODY_EXECUTE_3 명령을 안전하게 수행하고 양측 인계확인·차이 조정의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.를 완료한다.",
    "actorResponsibilities": [
      "VERIFIER 액터가 권한·업무분리 정책에 따라 양측 인계확인·차이 조정 관리자 업무 화면 업무를 수행한다."
    ],
    "entryConditions": [
      "거점, 운송수단, 인계 당사자와 계량 규칙이 등록되어 있다.을 충족하고 현재 상태가 STEP_2_COMPLETED이며 서버가 테넌트·프로젝트·액터 권한을 확인한 경우 진입한다."
    ],
    "exitConditions": [
      "양측 인계확인·차이 조정의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_3_COMPLETED 상태로 원자적으로 전이한다."
    ],
    "states": [
      "STEP_2_COMPLETED",
      "LOADING",
    ...
    "completionRule": "양측 인계확인·차이 조정의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_3_COMPLETED 상태로 원자적으로 전이한다.",
    "extensions": {
      "contractId": 3737,
      "sharedRuntime": true
    }
