...
export const screen_auto_97b0b176b7bfb7774752 = {
  ...
  "specification": {
    ...
    "businessPurpose": "프로젝트 종료·보관·재개·취소 프로세스의 종료·보관·재개·취소 실행 단계에서 APPROVER 액터가 PROJECT_LIFECYCLE_CONTROL_EXECUTE_4 명령을 안전하게 수행하고 종료·보관·재개·취소 실행의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.를 완료한다.",
    "actorResponsibilities": [
      "APPROVER 액터가 권한·업무분리 정책에 따라 종료·보관·재개·취소 실행 관리자 업무 화면 업무를 수행한다."
    ],
    "entryConditions": [
      "전환 요청자와 사유, 미결 태스크, 보존 정책이 확인된다.을 충족하고 현재 상태가 STEP_3_COMPLETED이며 서버가 테넌트·프로젝트·액터 권한을 확인한 경우 진입한다."
    ],
    "exitConditions": [
      "종료·보관·재개·취소 실행의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_4_COMPLETED 상태로 원자적으로 전이한다."
    ],
    "states": [
      "STEP_3_COMPLETED",
      "LOADING",
    ...
    "completionRule": "종료·보관·재개·취소 실행의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_4_COMPLETED 상태로 원자적으로 전이한다.",
    "extensions": {
      "contractId": 3795,
      "sharedRuntime": true
    }
  ...
  "traceability": {
    "contractId": 3795,
    "requirementIds": [
      "PROJECT_LIFECYCLE_CONTROL:PROJECT_LIFECYCLE_CONTROL_S4:ADMIN"
    ],
