...
export const screen_auto_53bf73851286cc8ad3d2 = {
  ...
  "blueprintCode": "BP_AUTO_53BF73851286CC8AD3D2165F",
  ...
  "specification": {
    ...
    "businessPurpose": "이의신청·분쟁·감사 대응 프로세스의 원본 증적 동결·독립 배정 단계에서 AUDITOR 액터가 APPEAL_DISPUTE_AUDIT_EXECUTE_2 명령을 안전하게 수행하고 원본 증적 동결·독립 배정의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.를 완료한다.",
    "actorResponsibilities": [
      "AUDITOR 액터가 권한·업무분리 정책에 따라 원본 증적 동결·독립 배정 관리자 업무 화면 업무를 수행한다."
    ],
    "entryConditions": [
      "대상 결정, 신청 자격, 기한과 원본 증적이 확인된다.을 충족하고 현재 상태가 STEP_1_COMPLETED이며 서버가 테넌트·프로젝트·액터 권한을 확인한 경우 진입한다."
    ],
    "exitConditions": [
      "원본 증적 동결·독립 배정의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_2_COMPLETED 상태로 원자적으로 전이한다."
    ],
    "states": [
      "STEP_1_COMPLETED",
      "LOADING",
    ...
    "completionRule": "원본 증적 동결·독립 배정의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.을 검증하고 결과·버전·감사 증적을 저장한 뒤 STEP_2_COMPLETED 상태로 원자적으로 전이한다.",
    "extensions": {
      "contractId": 3713,
      "sharedRuntime": true
    }
  ...
  "traceability": {
    "contractId": 3713,
    "requirementIds": [
      "APPEAL_DISPUTE_AUDIT:APPEAL_DISPUTE_AUDIT_S2:ADMIN"
    ],
