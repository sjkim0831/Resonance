...
export const screen_auto_45467a408513e83d9917 = {
  "id": "auto-45467a408513e83d9917",
  "blueprintCode": "BP_AUTO_45467A408513E83D99175E7D",
  ...
  "pageName": "원인분석·시정·재가동 승인 사용자 업무 화면",
  ...
  "specification": {
    ...
    "businessPurpose": "누출·사고·비상대응 프로세스의 원인분석·시정·재가동 승인 단계에서 COMPANY_MANAGER 액터가 LEAKAGE_INCIDENT_RESPONSE_EXECUTE_4 명령을 안전하게 수행하여 다음 완료 기준을 달성한다: 원인분석·시정·재가동 승인의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다.",
    "actorResponsibilities": [
      "COMPANY_MANAGER 액터가 권한·업무분리 정책에 따라 원인분석·시정·재가동 승인 사용자 업무 화면 업무를 수행한다."
    ],
    "entryConditions": [
      "다음 프로세스 시작 조건을 충족한다: 비상연락망, 경보 임계치, 대응 절차와 책임자가 승인되어 있다. 현재 상태는 STEP_3_COMPLETED이며 서버가 테넌트·프로젝트·액터 권한을 확인한 경우에만 진입한다."
    ],
    "exitConditions": [
      "다음 완료 기준을 검증한다: 원인분석·시정·재가동 승인의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다. 결과·버전·감사 증적을 저장한 뒤 STEP_4_COMPLETED 상태로 원자적으로 전이한다."
    ],
    "states": [
      "STEP_3_COMPLETED",
      "LOADING",
    ...
    "completionRule": "다음 완료 기준을 검증한다: 원인분석·시정·재가동 승인의 필수 입력, 권한, 증적, 상태 전이가 모두 검증되어야 한다. 결과·버전·감사 증적을 저장한 뒤 STEP_4_COMPLETED 상태로 원자적으로 전이한다.",
    "extensions": {
      "contractId": 3770,
      "sharedRuntime": true
    }
  ...
  "traceability": {
    "contractId": 3770,
    "requirementIds": [
      "LEAKAGE_INCIDENT_RESPONSE:LEAKAGE_INCIDENT_RESPONSE_S4:USER"
    ],
